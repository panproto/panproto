...
Every programming language is a theory in the GAT system. A program is a model (schema) of that theory. panproto's existing language protocols represent type-level structures (structs, interfaces, enums) as schema graphs. This chapter extends that to full programs: functions, statements, expressions, control flow, and module structure.

## The principle

A TypeScript file is not just a collection of type declarations. It contains functions with bodies, if/else branches, for loops, try/catch blocks, function calls, and expressions. All of these are structural elements that can be represented as vertices and edges in a schema graph. The only difference from type-level schemas is the number of vertex kinds and edge rules.
...
The walker captures the text between named children (keywords like `function`, punctuation like `{` and `}`, whitespace, and comments) as interstitial constraints with byte positions. The emitter collects all fragments (interstitials + leaf literals), sorts by byte position, and concatenates:
