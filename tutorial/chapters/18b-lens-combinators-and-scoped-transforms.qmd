...
## What combinators cannot express {#sec-limitations}

The compact transcript encoding from @sec-scoped-transforms (delta encoding, unit conversion, exploding arrays into parallel flat arrays) is procedural. These operations break the lens laws because they involve computational effects (arithmetic, array restructuring) that are not natural transformations on the presheaf topos. The right place for these is the expression language (`panproto-expr`), composed with `CoerceSort` or `ComputeField` field transforms (@sec-value-transforms), not the structural protolens algebra.

The boundary is clean: combinators handle structural graph rewriting (add, drop, rename, scope); the expression engine handles value-level computation.


## What comes next {#sec-combinators-bridge}

These combinators are the building blocks that the version control engine chains automatically when you run `prot data migrate`. The next chapter shows how versioned schemas, data, and complements are stored together in a content-addressed DAG, making every migration reversible without manual complement management.
