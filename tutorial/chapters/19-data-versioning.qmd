...
`schema migrate` walks the commit DAG, derives a protolens chain for each step, and applies the composed chain to every record. Formally, given a data set $D$ conforming to schema $S_i$ and a target schema $S_j$, the command computes the DAG path $S_i \to S_{i+1} \to \cdots \to S_j$, constructs the composed protolens chain $\pi_{j-1} \circ \cdots \circ \pi_i$, and applies the resulting lens $\ell = \text{inst}(\pi, S_i)$ to each record $d \in D$. Complements are stored so the process is reversible.

Consider a repository with three commits. Each commit introduced a schema change: a field was renamed, a vertex was added, an edge kind changed. Your `records/` directory still holds data written against the first schema. The data is *stale*; it conforms to a schema version that no longer matches HEAD. For three commits, manual migration is manageable. For thirty commits across five branches with merges, it is not.

## `schema add --data`: staging data alongside schema {#sec-add-data}
...
::: {.callout-caution}
## What happens if you stage data without a schema change?

`schema add --data records/` snapshots data bound to the current schema version. If you haven't changed the schema, does this produce a redundant snapshot, or does the content-addressing deduplicate it?
:::

::: {.callout-tip collapse=true}
## Answer

The content-addressing deduplicates it. `schema add --data records/` computes the blake3 hash of the data snapshot. If an identical snapshot already exists in the object store (because the data has not changed since the last commit), no new object is created. The commit records the same `data_id` as before. Storage cost is zero for an unchanged data set.
:::

## `schema migrate`: the one command you need {#sec-schema-migrate}
...
The `--dry-run` flag shows what *would* happen without writing anything:
...
## Complement storage: backward migration without data loss {#sec-complement-storage}
...
When you run `schema migrate records/ --backward`, the system:
...
## `schema status --data`: knowing when data is stale {#sec-data-staleness}
...
If `records/` was migrated to HEAD on `main`, and you switch to a `feature` branch whose HEAD is three commits ahead of `main`, does `schema status --data` report staleness relative to `feature`'s HEAD or `main`'s?
:::

::: {.callout-tip collapse=true}
## Answer

Staleness is always relative to the current branch's HEAD. If `records/` was migrated to HEAD on `main` and you switch to `feature` (whose HEAD is three commits ahead), `schema status --data` reports staleness relative to `feature`'s HEAD. The schema_id stored in the data snapshot is compared against the current HEAD's schema, regardless of which branch created the snapshot.
:::

## `schema checkout --migrate`: switching branches with data {#sec-checkout-migrate}
...
Without `--migrate`, `schema checkout` switches the schema but leaves data untouched. Subsequent `schema status --data` will show the data as stale.

## `schema merge --migrate`: merging with data {#sec-merge-migrate}
...
## Protocol versioning: pinning the metaschema {#sec-protocol-versioning}
...
3. Use `schema migrate --dry-run` to estimate complement sizes for a chain of five schema changes. Which steps are lossless? Which produce complements?
...
5. Write a CI script that runs `schema status --data records/` and fails the build if any data is stale.
