...
panproto's architecture has a recursive quality that turns out to be one of its most consequential features. The system that describes schemas is itself described by a schema. The theory language is itself a theory. And the 76 protocol definitions that panproto supports are all composed from a small catalog of reusable building-block theories.

This chapter covers both: the self-description architecture that makes panproto closed and extensible, and the building-block catalog that makes protocol definitions compositional. The compositional approach follows [GATlab](https://algebraicjulia.github.io/GATlab.jl/) [@lynch2024].
...
The bottom of the stack is approximately 2,000 lines of Rust that implement the core GAT machinery: sorts, operations, equations, theory mappings, composition, and model checking. This is the only part of panproto that is not data-driven. It is the interpreter for everything above.
...
A *model* of ThGAT is a concrete theory---ThGraph, ThConstraint, or any protocol theory. This is not a metaphor; it is a literal statement about the GAT formalism. The same `Theory` type that represents ThGraph also represents ThGAT, and the same model-checking machinery that validates a schema against ThGraph validates a theory against ThGAT.
...
This is the sense in which panproto is a general-purpose schematic version control system. It is not general because it supports every protocol (it does not, yet). It is general because the mechanism for adding a new protocol is the same mechanism that the existing protocols use, and that mechanism is itself described within the system.
...
The distinction matters: structural properties propagate automatically through migrations and theory composition, while validation constraints must be checked separately.
