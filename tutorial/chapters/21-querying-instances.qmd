...
Every schema instance is a graph: vertices carry data, edges encode relationships. The previous chapters showed how to build instances, version them, and migrate their data when the schema changes. But at some point you want to *ask questions* about the data: which posts got the most engagement, which annotations belong to a particular layer, which nodes satisfy some computed condition. That is what the query engine is for.

panproto provides two pieces that work together. The first is a Haskell-style expression language for writing predicates, projections, and computed values. The second is a declarative query engine (`InstanceQuery`) that uses those expressions to select, filter, and reshape nodes from a schema instance. This chapter covers both.

## The expression language {#sec-expression-language}

The expression language is a small functional language designed to feel natural if you have written Haskell, ML, or even spreadsheet formulas. It is used everywhere panproto needs to express computation over values: query predicates, field transforms (as seen in @sec-value-dependent-transforms), computed fields, and conditional survival.
...
This reads: for each `x` drawn from `xs`, if `x > 0`, yield `x + 1`. Multiple generators and guards can be combined:
...
The query engine operates on `WInstance` values (W-type instances, the tree-shaped data that conforms to a schema). An `InstanceQuery` is a declarative description of what you want: which vertex type to start from, which edges to follow, what conditions to check, and which fields to return. The engine evaluates the query in a fixed pipeline.
...
Path navigation is especially useful when you want to scope a query to a particular subgraph. Without it, anchoring on `"annotation"` would return every annotation in the instance. With a path, you can restrict to annotations reachable through a specific edge sequence.
...
List comprehensions in the expression language are useful for computed predicates and transforms, not just standalone evaluation. You might use a comprehension inside a `ComputeField` to derive a summary value:
