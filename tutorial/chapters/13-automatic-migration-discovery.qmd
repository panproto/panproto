...
A migration $F: S_1 \to S_2$ is a structure-preserving map (@sec-protocols-as-parameters for the definition). Finding one is a constraint satisfaction problem:

- **Variables**: one per vertex in $S_1$
- **Domains**: for each variable, vertices in $S_2$ with compatible kind
...
The search uses **MRV** (Minimum Remaining Values) to assign the most constrained vertex first. A vertex with only one compatible target gets assigned immediately. A vertex with many options is deferred.

**Forward checking** propagates constraints after each assignment. If any neighbor's domain becomes empty, backtrack immediately without exploring further.
...
The search returns all valid migrations, sorted by quality score. Quality combines:

- **Name similarity**: edit distance between mapped vertex IDs
- **Edge name preservation**: fraction of edges that keep the same label

The highest-quality migration is the most natural one.
...
:::{.callout-caution}
## Exercise: Limitations of automatic discovery
...
When extend pushes data forward, the target schema's equational constraints may not be satisfied. panproto enforces constraints by iterating:^[This iterative enforcement is an instance of the *chase algorithm* from database theory, adapted to panproto's equational theories. See [Appendix A](../appendices/A-formal-foundations.qmd).]
