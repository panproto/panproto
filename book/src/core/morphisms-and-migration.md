...
When a migration crosses a project boundary rather than a single file, the schemas it operates on are assembled from the working tree by walking a per-file Merkle structure. Panproto-vcs stores each parsed file as a [`FileSchema`](https://docs.rs/panproto-vcs/latest/panproto_vcs/object/) leaf and joins the leaves under a [`SchemaTree`](https://docs.rs/panproto-vcs/latest/panproto_vcs/object/), and the migration engine consumes the [`FlatSchema`](https://docs.rs/panproto-vcs/latest/panproto_vcs/object/) obtained by walking the tree. The decomposition is invisible at the level of the morphism: a migration speaks of one vertex and edge space, not of files. What it buys is dedup at storage time. Two commits that touch a single file share every other leaf, and the blob-OID cache keyed by `(git2::Oid, protocol)` skips re-parsing on the unchanged ones. The chapter on [Objects, refs, and the DAG](../vcs/objects-and-dag.md) develops the storage side.
