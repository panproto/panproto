...
The [previous chapter](./schemas-as-instances.md) identified a panproto protocol with a generalised algebraic theory and a schema with a model of that theory. What happens, then, when the theory changes? Every protocol version bump, every refinement, every translation between a subset and a superset of a schema language is a change of theory, and each such change induces a canonical way to move models (that is, schemas) and instances (that is, data) along with it. The mathematical account is *functorial data migration*, the framework of David Spivak [@spivak2012functorial]. This chapter walks through the account and lines it up with panproto's [`panproto-mig`](https://docs.rs/panproto-mig/latest/panproto_mig/) crate.

The chapter opens with theory morphisms, which lift the map-between-categories idea from the [Functors chapter](../foundations/functors.md) into the setting of [GATs](../foundations/gats.md). It then develops the three migration functors a theory morphism induces on the categories of models. A concrete example walks through a protocol extension from a single-field address record to a two-field one; for each migration functor, the example displays what happens on the instance side. The chapter closes with a note on how panproto packages these functors as the first-class migrations the engine manipulates. [The restrict/lift pipeline](./restrict-lift.md), the next chapter, takes the packaging apart stage by stage.
...
$\Sigma_f$ applied to a $T_1$-schema $M$ is the schema of the same people with a *freely added* email field: concretely, the population of $\Sigma_f M$ contains one entry for every pair (person of $M$, email address), with the email address ranging over all possible strings. This is the smallest $T_2$-model from which $M$ is recoverable by pullback, and it is rarely the operation a developer actually wants; the free addition is an invariant of the construction, not a design choice.
