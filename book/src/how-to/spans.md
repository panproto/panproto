...
- **Expecting `find_morphisms` to hand back the whole hom-set.** It returns the morphisms attaining the optimum, bounded by an engine cap of 1024 that applies to every request rather than only to `max_results = 0`. Reading `results[0]` gets what it always got; walking the list for a second-best alternative finds nothing, because there is no k-best over distinct quality levels.
