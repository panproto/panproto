...
Choose a span search when two schemas overlap but no total morphism embeds one in the other. A *span* is a pair of schema morphisms from a shared middle, written $\mathit{src} \leftarrow A \to \mathit{tgt}$; its **apex** $A$ is the part of the source that found a target. This is the common case in the measured corpus: 5117 of 5852 ordered lexicon pairs admit no total morphism. [Migrations as morphisms](../explanation/migrations-as-morphisms.md#spans-and-why-partiality-is-the-ordinary-case) gives the test and breakdown.
...
- `apex_pointed` says whether the apex has an entry point. [`induce`](https://docs.rs/panproto-schema/latest/panproto_schema/induce/fn.induce.html) never synthesizes one, so an apex with no entries is recorded rather than repaired.
...
- **Reading an empty morphism list as proof that no total morphism exists, without checking the return.** It is that, and only when the call succeeded. A search that could not be posed reports `SpanError::Build`, and one that spent its budget before reaching any complete assignment reports `SpanError::Stopped` naming the limit that ran out, so the empty answer is a statement about the pair and a refusal is a statement about the budget.
...
- **Expecting `find_morphisms` to hand back the whole hom-set.** It returns the morphisms attaining the optimum, bounded by an engine cap of 1024 that applies to every request rather than only to `max_results = 0`. Walking the list for a second-best alternative finds nothing, because there is no k-best over distinct quality levels: every element is a tie for first.
