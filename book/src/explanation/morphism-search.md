...
The span carries both measurements and a certificate. `quality` reads the assignment's primary cost, while `apex_coverage` records the fraction of source vertices retained. `quality_bounds` converts the solver's lower and upper cost bounds into the corresponding interval on the higher-is-better quality scale.
