...
The obvious way to run the search is to list every structure-preserving map from one schema into the other, score each one, and keep the best. panproto did exactly that until recently, and it failed in two ways that turned out to share a cause. Cost was linear in the number of complete maps rather than in the size of the schemas, so one pair of shipped [ATProto](https://atproto.com/) lexicons did not finish in fifteen minutes. Alignment hints were written into the solver as hard restrictions on where a vertex could go, which meant a more permissive tier of hints could fail on a pair that a stricter tier had aligned: two individually plausible hints can be jointly impossible, and nothing was checking the conjunction. Both symptoms follow from treating the search as an enumeration to be filtered rather than as an objective to be minimised.

The search is now an exact optimisation. One decision variable per source vertex; its candidate values are the kind-compatible target vertices plus one extra value meaning "leave this vertex out of the answer"; every local choice carries a numeric cost; and the algorithm computes the cheapest complete assignment without listing the others. Hints enter as costs to be lowered rather than as candidates to be removed, so they change which answer is best without changing which answers exist. The apparatus that makes all this precise dates from the 1980s and 1990s and comes with theorems attached, which is most of the reason for adopting it rather than inventing something.
...
$$S(k) \;=\; \langle\, \{0, 1, \ldots, k\},\; \oplus,\; \succeq \,\rangle, \qquad a \oplus b = \min(k,\, a + b), \qquad \bot = 0, \qquad \top = k,$$

a valued CSP in the sense of @schiexfargierverfaillie1995valued, whose contribution was to replace the ad hoc aggregation operators of earlier soft-constraint proposals with a single ordered commutative monoid subsuming classical, possibilistic, weighted and probabilistic constraint satisfaction. A parallel presentation, the c-semiring framework of @bistarellimontanarirossi1997semiring, reads the two operations as constraint projection and constraint combination. The two are close relatives rather than notational variants, and nothing below rests on their being interchangeable. We take the valued-CSP orientation, so that $\bot$ is the best cost, $\top$ is infeasible, and $\succeq$ reads "worse than or equal".

The axioms are commutativity, associativity, and three more:

$$a \oplus \bot = a, \qquad a \oplus \top = \top, \qquad (a \succeq b) \implies (a \oplus c \;\succeq\; b \oplus c).$$
...
$$(u \oplus w) \oplus (v \ominus w) \;=\; u \oplus v \qquad \text{whenever } w \preceq v$$

[@cooperschiex2004arc]. That identity is the single load-bearing fact of the whole solver: every operation that shifts cost from one table to another subtracts exactly what it adds, so every proof that such an operation is harmless is an instance of it. An operation satisfying it only approximately makes the optimiser unsound rather than imprecise, which is the next section.
...
The objective is lexicographic: minimise the quality cost, then minimise the number of dropped source vertices. Both components live in one integer,

$$\mathrm{Cost} \;=\; q \cdot \rho + \delta, \qquad \rho = \bigl(|V_S| + 1\bigr) \text{ rounded up to a power of two},$$
...
Write $C_S = \{\, v \in V_S : v \text{ has at least one named outgoing edge} \,\}$ for the SFN normaliser of the overlap component, and write $P$ for the reference normaliser, the set of mapped pairs $(s, t)$ where either side has a named outgoing edge.

**Proposition.** On a total morphism the two numerators agree, so the two versions of the component differ only in their normaliser.
...
*Proof.* The anchor pool at $\mathcal{T}'$ contains the pool at $\mathcal{T}$, and the aggregate is monotone in the pool by hypothesis, so $\mathrm{conf}_{\mathcal{T}'}(v, a) \geq \mathrm{conf}_{\mathcal{T}}(v, a)$ pointwise. The anchor term is decreasing in confidence, so every unary entry is pointwise no worse at $\mathcal{T}'$, and by monotonicity of $\oplus$ no assignment's total is worse either. The two minimisations range over the same set by tier invariance, so the minimum at $\mathcal{T}'$ is at most the minimum at $\mathcal{T}$. $\square$

Both theorems are about the objective, and neither says the *alignment* improved. A tier that fires more strategies is guaranteed a cost no higher and an apex no smaller; whether the correspondence it names is more nearly correct is a separate claim, and one the weights have never been calibrated to support. The shipped anchor weight is also zero, which makes the term the same constant on every value of every variable, so tier invariance is the property with teeth today and tier monotonicity is checked in CI with the weight raised. Both are properties of the encoding rather than of the current defaults, which is what lets the weight be raised without reopening the question. The obvious alternative encoding, a penalty for declining an anchored target, gives the wrong monotonicity: more strategies means more penalties, hence a worse optimum, and the ladder would run backwards. That is why ROE is stated as a prohibition rather than left to taste.

## The primal graph and its width

The **primal graph** of the network has one node per variable and an edge between two variables that share the scope of some cost function. An elimination sequence is an ordering of the variables; eliminating a variable adds the edges that make its not-yet-eliminated neighbours a clique; and the **induced width** of the sequence is the largest number of not-yet-eliminated neighbours any variable has at the moment it is eliminated. Width is the parameter that decides everything about cost here. @dechter1999bucket is the paper that says so: it unifies directional resolution, adaptive consistency, Fourier and Gaussian elimination, dynamic programming for combinatorial optimisation and several probabilistic inference tasks as one algorithm under different combination operators, all of them exponential in time and space in exactly this quantity.

Two deterministic orders are computed and the narrower is taken: a min-fill ordering, and the reverse of ascending source vertex name order, which puts the deepest source vertices first and holds the width at one on a tree. The resulting width is then priced against a memory and operation budget, and that price is what the dispatcher routes on.
...
decode(order, buckets):
  for p = n-1 .. 0:                     # backwards along the elimination sequence
    X ← order[p]
...
Inputs whose width does not price inside the budget go to branch and bound, held to the same standard: the optimum it returns is the optimum, and the assignment it returns achieves that optimum when scored against an untouched copy of the network.
...
The largest domain is measured on the wider population too, and it is a distribution rather than a headline. Taking the largest single domain of each ordered pair, with $\bot$ excluded, gives a minimum of 1, a median of 5, a 95th percentile of 12 and a maximum of 18. Taking every source vertex separately, over the 74176 variables the 5852 pairs pose between them, gives a minimum of 0, a median of 2, a 95th percentile of 9 and the same maximum of 18. That maximum is a thin tail: 75 of the 5852 pairs reach it, every one of them searching into `dev.panproto.vcs.commit`. At $w \leq 2$ the sweep costs $d^{w+1}$ operations in the largest domain $d$, so the widest pair in the corpus prices at $18^{3}$ operations per eliminated variable, orders of magnitude inside the default budget of $10^{9}$ combine operations and 64 MiB of tables. That is why exact inference is the primary path rather than a fortunate one.

Three further observations shaped the design, and all three were taken from a single profiled pair before anything was measured across the corpus. Two have since been measured over all 5852 ordered pairs, and one of those two came back other than the pair suggested. The third is a profile of code this branch deleted, and it stays qualitative for that reason.

Partiality is the ordinary case, and it is now measured rather than inferred. On the profiled pair, six of `app.bsky.feed.post`'s 39 source vertices, the four `array` and two `integer` ones, had empty domains against `app.bsky.actor.profile`, which holds neither kind, so no total morphism between those two exists at all. Over the whole corpus, 4950 of the 5852 ordered pairs contain at least one source vertex with no kind-compatible target, which is 84.6% of them. That condition is sufficient for an empty hom-set and not necessary, and the gap between the two is worth carrying: a further 167 pairs have every domain non-empty and are emptied by naturality alone, so 5117 pairs, 87.4%, admit no total morphism and only 735 admit one. Reporting the 84.6% by itself understates the rate, because it quotes a lower bound as though it were the answer.

The hom-set was the full Cartesian product of the domains on the profiled pair: every combination of assignments to the unconstrained vertices was a valid morphism, and naturality ruled out nothing. [`detect_product`](https://docs.rs/panproto-mig/latest/panproto_mig/solve/elim/fn.detect_product.html) is what reads that shape off a network, and it needs no sweep at all: it reports when every constraint is universal, from which the count follows as the product of the domain sizes. The solution-count instantiation of the sweep is the separate and more expensive reading, and it is the one carrying an enumeration ceiling; a saturated count and a saturated product compare equal whatever the constraints did, so the domain snapshot reads the shape off `detect_product` and uses the count for the single bit of whether it is zero, which saturation cannot corrupt.

The corpus does not repeat the profiled pair. Of the 735 ordered pairs admitting a total morphism, 327 have a hom-set that is the full product, which is 44.5%; on the other 408, some constraint forbids something. Reading the same numerator against the wider denominator, the 902 pairs whose every domain is non-empty, gives 36.3%, the difference being the 167 pairs that belong to that denominator and to neither part of the first ratio. Naturality thus constrains nothing on a substantial minority of the pairs where it has anything to constrain, and constrains something on the majority of them. Which way that reads depends on what is being asked: on those 327 pairs, "there is a morphism between these two schemas" is close to a free statement, and on the other 408 it is not.

Propagation was a rounding error in the old search's runtime, and this is the observation still resting on a profile. That search spent nearly all of one pair's time recomputing a quality function over a small set of distinct entries, and almost none of it on the forward checking that was supposed to be doing the pruning, because most of its nodes reached a solution and there was nothing to prune. The percentages behind those adverbs are deliberately not quoted. They were read off a profile of code this branch deleted, nothing in the tree reproduces them, and the work they measured, forward checking inside an enumerate-and-filter search, is not the work the current design does. A profile of deleted code can motivate a rewrite, which is what the timings above are doing; it cannot evaluate what replaced it.

## Where the measurements contradict the recommendations

Four literature surveys were commissioned on this problem before the rewrite. All four identified optimisation-by-enumeration as the root cause, and all four are right about that. All four also put propagation strength, in the form of arc consistency, bitset domains and supplemental graphs, at or near the top of their recommendations.

Those recommendations are correctly grounded. @freuder1982sufficient relates the width of the constraint graph to the level of consistency that makes search backtrack-free, and three of the four surveys made that their headline. @dechterpearl1987network gives algorithms optimal on sparse and tree-structured networks and derives value-ordering advice by relaxing the pending subproblem to a tree and counting its consistent solutions. @regin1994filtering achieves generalised arc consistency for the all-different constraint by matching theory. @mccreeshprosser2015backjumping adds supplemental graphs that generate implied constraints, alongside bit and thread parallelism and a lazy conflict-directed backjumping compatible with parallel search.

The measurements disagree for this instance class, and we follow the measurements. Three of them bear on the recommendation, and each is a snapshot rather than an impression.

First, the propagation the surveys recommend lives on the branch and bound path, and this corpus does not reach it. All 861 rows of the answer snapshot came back from exact inference, and over the 5852 ordered pairs no network measures an induced width above two, at which the message tables price orders of magnitude inside the default budget. A constant-factor win on a path nothing takes is worth nothing, and bit-parallel propagation, which one survey called the largest available constant-factor win, is exactly such a win. Second, full arc consistency would guarantee a backtrack-freeness these networks already have: the forward sweep of elimination is backtrack-free by construction at any width it can afford, which here is every width the corpus exhibits. Third, on 327 of the 735 pairs admitting a total morphism every constraint is universal, so on those no propagator at any strength has a value to remove; on the remaining 408 it does, which is a real qualification on an argument the profiled pair alone would have made unconditionally. The largest constant-factor win actually available in the old search was memoising the quality function, and the decomposition subsumes that entirely.

What has been dropped from this argument is the profile. It measured forward checking inside an enumerate-and-filter search, while the propagation the surveys were recommending is EDAC\* enforcement inside a cost function network; the two are different work in different programs, so the profile was never evidence about the code that replaced it, and nothing above now rests on it. The caveat that survives is a different one: the width, the domain sizes and the product structure are properties of one corpus of 77 ATProto lexicons, measured rather than proved, and a corpus is not a theorem.

The disagreement has a cause worth recording rather than smoothing over: the surveys were briefed on an instance roughly five times the size of the one that exists, around 200 source vertices against 100 target vertices, where the measured pairs pose between 2 and 39 variables and the profiled pair ran 39 against 15. Their reasoning is sound for the instance they were given. If schema sizes grow, or if a protocol arrives whose schemas are genuinely dense rather than near-forests, the recommendation comes back into force, which is why the branch and bound path exists and is held to the same correctness standard as the elimination path rather than being a degraded mode.
