...
`quality` ranks spans over *one* source schema and nothing else, because every denominator of the objective is fixed by `src`. The two empty cases read oppositely: an empty apex over a non-empty source scores `0.0`, since every vertex paid the drop cost, and an empty apex over an empty source scores `1.0`, since nothing paid anything. Both say that the schemas share nothing, which is why a caller ranking pairs reads `apex_coverage` alongside the score.
