...
| Conditional and match | `if p then a else b`; `case value of { pattern -> body }` |
...
| String | `Concat`, `Len`, `Slice`, `Upper`, `Lower`, `Trim`, `Split`, `Join`, `Replace`, `Contains` | `Len` counts UTF-8 bytes; `Slice` indexes Unicode scalar values. `Contains` also tests list membership. |
...
| Instance traversal | `Edge`, `Children`, `HasEdge`, `EdgeCount`, `Anchor` | The standard evaluator returns `Null`. Use `panproto_inst::instance_env::eval_with_instance` for instance-aware behavior. |
