...
The root workspace manifest is `Cargo.toml` at the repository root; the individual crates are under `crates/`. The non-Rust packages — the TypeScript SDK and the Python SDK's Python-side code — live under `bindings/typescript/` and `bindings/python/`.
...
The SDK bindings layer is what non-Rust callers touch. The WASM boundary is in [`panproto-wasm`](https://docs.rs/panproto-wasm/latest/panproto_wasm/), the Python PyO3 native binding is in [`panproto-py`](https://docs.rs/panproto-py/latest/panproto_py/), and the panic-safe C ABI used by the Haskell binding lives in [`panproto-c`](https://docs.rs/panproto-c/latest/panproto_c/) (with the cabal package itself under `bindings/haskell/`).
