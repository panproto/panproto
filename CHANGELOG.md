...
- **A release no longer withholds its binaries because a metadata job failed** (`.github/workflows/build-panproto-c-bindist.yml`): the job that generates the CycloneDX inventory was listed in the attach job's `needs`, and it invoked `cargo cyclonedx` with a `-p` flag that tool does not accept. So the inventory failed, the attach job was skipped, and the v0.73.0 tag published no C archives and no XCFramework at all; the Swift publish then waited two hours for an XCFramework that was never coming and left the package pinned to the previous release. An inventory that fails to generate is worth reporting and is never worth withholding the artifacts people install, so the attach job no longer depends on it and the attestation that consumes it is guarded on the file existing. The generator now names the file it expects rather than searching for one, since `cargo cyclonedx` writes one inventory per workspace member and a search over thirty-eight of them found `xtask`'s, which would have been attested and published as `panproto-c`'s; the file is also checked to describe the component it claims to. Recovering a release that failed this way needed the fix to be present in the run doing the recovery, which a tag cannot be given after the fact, so the workflow now takes a tag to build from and attach to: dispatching it from a branch runs the corrected file against the tag's source.

- **The two release jobs that flaked on consecutive releases are hardened** (`.github/workflows/ci.yml`, `python-wheels.yml`, `python-wheels-companions.yml`): the MSRV test job was cancelled mid-`nextest` on v0.60.0 and failed on v0.61.0 with `No space left on device` while compiling `panproto-grammars`, while the stable job passed on identical code both times. The margin was thin rather than the budget wrong, so both halves are attacked. More preinstalled tooling is trimmed before any cargo step, and the test builds now write `line-tables-only` debug info rather than full DWARF: full DWARF is the bulk of what a debug build of 250 vendored grammars produces, and a test job needs the file and line a backtrace reports, not the rest. Measured locally, the same build reached 3.9 GB where it had previously been heading for tens. The job also reports its remaining headroom afterwards, because a run that passes with nothing left is one that fails on the next grammar added, and nothing else would say so. Separately, a single `archive.ubuntu.com` failure in one of about fifty matrix legs hard-failed the companion wheel build on v0.61.0; because the publish gate needs every leg green, none of the ten companion wheels published until someone re-ran it by hand. Package fetches now retry with a backoff before they are allowed to fail.

### Features

- **Fuzz targets are run rather than only compiled, and the unsafe boundary gets sanitizers** (`.github/workflows/fuzz.yml`): CI ran `cargo check` over the fuzz workspace, which catches a target that has stopped compiling against the crate it drives but establishes nothing about the code the target exists to exercise. A target that has not executed an input has not fuzzed anything. All seven checked-in targets now run a bounded campaign nightly, seeded from the committed corpora and free to build inputs larger than any of them, and pull requests run a one-minute smoke on the two that read untrusted bytes. A crash is minimized before it is uploaded, since a raw libFuzzer artifact is mostly padding while a minimized one is a regression test. `AddressSanitizer` and `UndefinedBehaviorSanitizer` cover the grammar and parse crates, which is where this workspace's unsafe code is, and Miri covers the pure-Rust crates it can execute. The existing compile check stays as the fast failure mode.
...
- **One resource policy for every public surface** (`panproto-expr`, `panproto-protocols`): several subsystems already imposed useful local bounds, on parser walk depth, CST extraction depth, expression evaluation steps, morphism search and model-check assignment counts. Each was sound on its own, and together they were not a policy: what an input was allowed to cost depended on which door it came through, so the same document could be refused through one surface and accepted through another. `ResourceLimits` names the seven things a bounded operation can exhaust, with conservative defaults, and a `Budget` draws them down. Two properties make it a policy rather than another local bound. A budget is shared rather than per-call: cloning shares its counters, so a nested operation draws from the same allowance as the operation containing it and an oversized input cannot slip past by arriving in pieces. And a failure names which allowance ran out and what it was set to, since with seven of them "too deep" does not even say which setting to change. `parse_schema_document`, `parse_schema_source` and `parse_schema_bundle` apply the defaults, and every binding calls those, so the CLI, Python, C and WASM inherit one policy without a line changing in any of them. `_within` variants take a caller's budget; `ResourceLimits::unbounded` and a zero field remove bounds, which is a choice a Rust caller can make and which no FFI or command-line entry point selects, because unbounded behaviour should be asked for rather than inherited.
...
- **Every GitHub Action is pinned to a commit, and every release archive carries a checksum, an SBOM and an attestation** (`.github/workflows/`): third-party actions were referenced by mutable tags, so `@v2` was whatever the tag pointed at on the day a workflow ran, and a compromised or retagged action would have executed inside jobs holding `contents: write` and the OIDC identity that publishes to crates.io, npm and PyPI. All twenty-two actions across nine workflows now name a full commit SHA with the human-readable version in a trailing comment, which is the form Dependabot already understands and bumps. `release.yml` is deliberately left on version tags: cargo-dist generates it and the Release workflow fails on a tag when the file diverges from what `dist generate-ci` would produce, so a hand-edit there breaks every release; the Dependabot config now says so, since a bump reaching that file is the failure that has to be caught in review. On the artifact side, the XCFramework carried a checksum because SwiftPM verifies one itself, while the tarballs and zips carried nothing. Every archive now gets a `SHA256SUMS` entry, a CycloneDX SBOM records the crates and versions the C library was built from, and both the archives and the SBOM receive GitHub build attestations tying them to the workflow and source revision that produced them. Attestation is keyless, signed by the job's OIDC identity, so this introduces no publication secret. A checksum alone only says a file matches a hash published beside it, and an attacker able to replace one can replace the other; the attestation is what distinguishes an archive panproto built from one assembled elsewhere and named the same. `book/src/how-to/install/verify-artifacts.md` covers verifying all three.

- **quick-xml is upgraded past its two denial-of-service advisories** (`panproto-io`): `deny.toml` ignored `RUSTSEC-2026-0194` and `RUSTSEC-2026-0195` on the reasoning that this pathway uses a `Reader` without namespace resolution, which the second advisory says is unaffected, and that the first's quadratic duplicate-attribute scan is bounded by schema-guided local documents. That rationale held only while callers kept supplying non-adversarial input, which is not a property a library can promise about its own users. Both ignores are gone and `cargo deny check advisories` passes without them. The upgrade needed a real migration rather than a version bump, because 0.41 stopped folding references into the surrounding text: `a &amp; b` now arrives as three events rather than one, and code writing each text event straight onto the node kept only the last fragment. Character data is accumulated across events and written when its element closes, so text, character references, predefined entities and CDATA all reach the value. Two things are fixed on the way. CDATA was reaching a catch-all arm and being discarded, so an element whose whole content was a CDATA section read back with no value at all. And `trim_text` moves off the reader onto the assembled buffer: trimming each event separately would eat the spaces around every reference, turning `a &amp; b` into `a&b`. An entity the document defines itself is written back literally rather than expanded, since expanding one is what the billion-laughs class of attack turns on.

### Features

- **A cross-document reference resolves for OpenAPI, JSON Schema and Avro, not only ATProto** (`panproto-protocols`, `@panproto/core`, `panproto-py`): `parse_schema_bundle` resolved references across a set of documents for ATProto alone, while several other protocols had the same latent gap. Their parsers take one document, so a reference into a sibling resolved to an opaque placeholder vertex carrying no fields, and a lens or migration had nothing typed to bind to. Splitting a specification across files is the standard way a large API is organized, and multi-file Avro schema sets are ordinary, so the gap is reached by routine input. Each new parser follows the shape ATProto's established: every document's definitions are mapped to their vertex ids before any document's body is walked, so by the time a reference is resolved its in-bundle target already exists as a real, typed vertex. Avro resolves a named type addressed by its fullname, `namespace` plus `name`, which nothing did before: a field declaring `com.example.Address` bound to nothing, because vertices are keyed by the bare name. Enums are named types too and are registered the same way. Entry selection now ranges over the whole bundle, so pulling a sibling's definitions in does not promote them to basepoints. OpenAPI and JSON Schema resolve a `$ref` spelled against the target document's `$id`; a relative-path `$ref` is not resolved, because a bundle is an unordered array of documents with no filenames for a path to be relative to. Building the whole map before walking also resolves a `$ref` to a definition declared later in the same document, which the incremental map could not. Each single-document parser now delegates to its bundle parser over a one-element slice, so the two cannot drift apart. `list_bundle_parser_protocols` was exported from WASM but reached no binding; it is now `panproto.list_bundle_parser_protocols()` and `Panproto#listBundleParserProtocols()`, so a caller can ask which protocols resolve across documents rather than hard-coding a list that has grown and will grow again.

### Bug Fixes

- **An assembled project's property names are the ones its data uses** (`panproto-vcs`, `panproto-project`): assembling a multi-file project into one schema prefixed each file's vertex ids with the file's path, which is right, and prefixed each edge's *name* with it too, which is not. An edge's name is the key the property has in the data, scoped by a source vertex the prefix has already made unique, so an assembled project declared a property called `lexicons/com.example.record.json::item` where every record has a key called `item`. No record could match any property, and since required edges were rewritten the same way, every record read against a project schema came back missing every required field. In effect no data could be lifted through a project schema at all. Vertex ids are still prefixed, at all four sites that rewrite them; names are left alone. Found by staging data against a project schema, which nothing did before, because staging never read the data.

- **Staged data is read before it is staged, and a commit requires a check that finished** (`panproto-vcs`, `panproto-cli`, `panproto-py`): `add_data` read a file, counted its bytes as records, and stored them opaquely under a `schema_id`. It never parsed them, never checked they were JSON, and never validated them against the schema it was recording them under, so the index and the resulting commit asserted an association nothing had established. `record_count` was the clearest symptom, reporting 1 for a file that is not JSON at all. The stored bytes were also the file's own, while every reader of a data set (the migration path, `square`, `data_at`'s consumers) decodes MessagePack of `Vec<WInstance>`, so a set added this way could not be read by any of them and first failed at whatever later operation tried to lift or convert it, by which point the commit that introduced it is history. Staging now parses each record, lifts it at the schema's primary entry vertex, checks it against the schema's W-type shape and its attribute constraints, and stores the lifted instances. `AddDataOptions::skip_verify` skips the check but not the read, since a data set that names its schema cannot be recorded under one its bytes cannot be read against. Separately, the default commit path rejected `ValidationStatus::Invalid` but accepted `Pending`, so an object staged with `add --skip-verify` could be committed without `commit --skip-verify` ever being typed: "not checked" and "checked and passed" reached the same place. A default commit now refuses a pending schema or data set with `VcsError::ValidationPending`, and `commit --skip-verify` records what it waved through on the commit's `unverified` field, so a bypassed commit stays distinguishable from a verified one afterwards rather than only at the moment the bypass was taken.

- **Verification reports what it established, not merely what it found** (`panproto-gat`, `panproto-cli`, `panproto-vcs`): an empty violation list meant two different things, and `schema verify` read both as the same thing. A theory that failed to typecheck was printed and skipped; an equation check that returned an error, which is how an exhausted assignment budget arrives, was printed as incomplete. Neither contributed to `total_violations`, so the command went on to print `Verification passed.` and exit zero having checked nothing. That is a fail-open at a trust boundary: three outcomes, verified, violated and inconclusive, collapsed onto one boolean, and the inconclusive one landed on the wrong side of it. The new `verify_model` returns a `VerificationReport` carrying a `VerificationStatus` of `Passed`, `Failed` or `Incomplete`, and reports failure in that value rather than as an `Err`, since an `Err` is easy to print and step past. `Incompleteness` names which of the two ways a run stopped. `schema verify` aggregates per theory and takes the weakest outcome: one refutation fails the run, and one unchecked theory leaves the run unable to claim the schema verified however many others passed. Both exit nonzero, `--allow-incomplete` accepts an inconclusive run for exploratory use while still naming the theories that were not checked, and `--format json` carries the same three-valued status per theory and for the run, in theory-name order rather than hash order. The VCS staging path already recorded an incomplete check as an error rather than a note, so it failed closed; it now goes through the same API, which additionally typechecks the theory before checking equations against it.

- **A failed ATProto theory composition is reported rather than dropped** (`panproto-protocols`, `panproto-cli`, `panproto-c`, `panproto-wasm`, `panproto-vcs`): `register_theories` returned `()` and wrapped each `pushout_by_name` in `if let Ok(...)`, so a composition that failed left the five component theories registered and `ThATProtoSchema` or `ThATProtoInstance` simply absent, with nothing said. A caller then held a registry that looks initialized, and the schema theory it would have validated against was gone, so validation passed by having nothing to check. It now returns `Result<(), ProtocolError>`, and every theory including both composites is built before anything is written, so a failure leaves the destination registry exactly as it was rather than partially populated. `ProtocolError::TheoryRegistration` names the composition stage that failed and the error it failed with. The CLI, C, WASM, VCS and bench callers all propagate or deliberately handle it; the CLI's repository setup additionally reports a protocol whose named schema theory its own registration does not produce, which had the same silent effect of leaving commits validating structure only.

- **A case on an indexed sort requires only the constructors its index admits** (`panproto-gat`): exhaustiveness compared a case's branches against every constructor the scrutinee's sort declares, reading the sort's head and never its index. Sort checking then rejected the branch that demand had just required, because the constructor's output index would not unify with the scrutinee's. So `head(n, v : Vec(succ n))` had to carry a `nil` branch that it could not carry: without one the theory was refused for `missing branches for: ["nil"]`, with one for `cannot unify zero(...) with succ(...)`. The second error is good evidence for the first, since it shows the checker already knew `zero` cannot be `succ(n)` and simply did not consult that when deciding which branches were required. A constructor is now required exactly when its declared output sort unifies with the scrutinee's, and that is the same unification the branch checker runs, so the two can no longer disagree about whether a branch exists. A branch naming a constructor the index excludes is refused as `UnreachableCaseBranch` naming both sorts rather than as a unification failure, and a scrutinee whose index admits nothing is refused as `CaseOnUninhabitedIndex` rather than as a case missing every branch. A sort with no parameters carries no index and every constructor of it unifies, so non-dependent matching is unchanged. What this unblocks is elimination over an indexed family: `head` and `tail` on a non-empty vector, an evaluator over a universe of codes, and any total function whose domain an index narrows.
...
- **A ref name cannot address a file outside the store** (`panproto-vcs`): `FsStore` joined a ref name straight onto the store root, so a name carrying `..` or a leading `/` named a path anywhere on the host. Ref names arrive over the network, since `NodeClient::pull` writes every name the remote advertises, which made a remote's ref listing a write primitive against the machine that pulled from it. Every name-to-path helper now requires a relative name of ordinary components and reports `VcsError::InvalidRefName` otherwise.
- **The object store verifies what it reads and never leaves a torn write behind** (`panproto-vcs`): the store is content-addressed and never checked the address, so a truncated write, a corrupted file, or a substituted object came back as genuine. `get` now re-derives the object's ID from the bytes it decoded and refuses a mismatch with `VcsError::ObjectCorrupted`, and `put` stages into a sibling temporary file, flushes it, and renames it into place, so a reader sees either the previous contents or all of the new ones. The object-to-ID match that both stores had grown their own copy of now lives once, as `hash::object_id`.
- **A fetched object is checked against the id that asked for it** (`panproto-xrpc`): `NodeClient::get_object` decoded whatever the node sent and handed it back, so a remote could answer any request with any object and the client would file those bytes under an id that does not describe them. `pull` does exactly that for every id the negotiation names, so every later reader would then trust the wrong content. The object is now hashed and matched against the requested id, and a mismatch is refused with `XrpcError::ObjectMismatch` naming both.
- **The panic hook covers panproto's boundary and nothing else** (`panproto-c`): `pp_init` replaced the process-global panic hook with one that printed nothing, silencing every panic in the host process rather than only the ones panproto was about to convert into a status code. A host's crash reporter, its logger, and the Rust default all went quiet. The hook now silences a panic only while the panicking thread is inside `panic::guard` and forwards every other panic to whatever hook was installed before `pp_init` ran; installation is once-only, so repeated calls cannot grow the chain.
- **A companion grammar's tags query is checked for UTF-8 before tree-sitter reads it** (`panproto-py`): registration converted third-party bytes with `from_utf8_unchecked`, so an invalid sequence became a `str` violating the type's invariant and the query compiler then read it, which is a segmentation fault (observed as exit status 139). The bytes are now checked before any `'static` allocation is leaked and before the language pointer is used, and a bad entry is refused with a ValueError naming the grammar and the offending index. The `language_ptr` documentation also described the wrong pointer, telling callers to pass the address of the `tree_sitter_<name>` function rather than the `TSLanguage *` it returns.

### Features

- **A builtin is a value wherever its name is written** (`panproto-expr`, `panproto-expr-parser`): builtin names resolved only in the head position of an application, so the higher-order builtins could not be passed the functions the language ships: `map upper [...]`, `fold add 0 [...]`, `"hello" & upper` and `let f = upper in f "a"` were all `UnboundVariable`. The builtins now form a scope beneath the environment, so a free variable naming one denotes that builtin curried over its arity, while a lexical binding of the same name still shadows it. A builtin applied to fewer arguments than it takes evaluates to the function of the rest. The name table and the surface-to-`Expr` argument permutation move from the parser into `BuiltinOp`, so the evaluator and the parser read one definition rather than two.

### Bug Fixes

- **Schema diffs now preserve the changes they report** (`panproto-lens`, `panproto-inst`): the diff-to-protolens path compared only part of a schema and could emit a lens that did not reproduce the target. It now simulates the generated transforms and checks every schema component. Added and changed fields receive defaults through one ambiguity-checked synthesis pass; an explicit null remains distinct from no default; kind changes require a registered coercion and install the corresponding value transform; and metadata changes cascade onto the resulting schema. Restricting a W-type now identifies a direct arc by its full edge identity, so dropping one of two parallel edges cannot silently relabel the survivor.
- **A compiled lens keeps its ordered program at every boundary** (`panproto-lens-dsl`, `panproto-c`, `panproto-py`, `panproto-wasm`): composition merged transforms without retaining the order in which scoped stages run, while the C and WASM slabs stored only the underlying migration. A compiled lens now carries its ordered stages through composition, fusion and each native resource. Python's pipeline and composition entry points use that same representation. This preserves the difference between programs that touch the same field in different scopes or at different points in a pipeline.
- **CLI lens artifacts are executable after a round trip** (`panproto-cli`): `lens generate` serialized only a migration chain, losing transforms, ordered stages, extensions, auto-lens settings, symmetric chains and invertibility metadata. The versioned `panproto-compiled-lens-v1` artifact records the complete compiled lens, `lens apply` recognizes it before the legacy chain and schema forms, and `lens compose` concatenates real artifact steps while rejecting mixed or synthetic inputs. `lens verify` now requires both data and its schema and passes the data to the law checker, rather than accepting a schema where the data argument belonged.
- **Expression and query values cross the TypeScript/WASM boundary without changing shape or integer value** (`@panproto/core`, `panproto-wasm`): the wrapper forwarded recursive expression, pattern and literal values as though their TypeScript and Rust encodings were identical, and JavaScript numbers silently rounded large `i64` values. Recursive wire conversion now covers every expression and query position. Integer literals accept `number | bigint`, reject unsafe numbers and values outside the signed 64-bit range, and decode to a number only when that is exact. Query functions use the schema-handle ABI while the raw schema-byte entry points remain available.
- **A Haskell cascade uses the migration the engine compiled** (`panproto-haskell`): the induced-migration path compiled the migration again, used a reconstructed target and could leak raw handles when a later conversion raised an exception. It now uses the engine-provided migration handle and target schema and releases each raw handle on every exceptional path.
- **Swift uses the engine's canonical migration and complement carriers** (`bindings/swift`): the compiled-migration wrapper keyed a hyper-resolver only by its hyper-edge name, which discarded the source fan shape, and encoded the table as a CBOR map where Rust expects an array of key-value pairs. The data-set wrapper also rewrote a complement's pair arrays back into maps before sending them to the C boundary. Swift now preserves the `(hyper-edge, labels)` key and passes complement pair arrays unchanged. Its captured engine fixtures have been regenerated against those representations.
- **The Python extension is testable as a Rust workspace member** (`panproto-py`): enabling PyO3's `extension-module` feature for every workspace build suppressed the libpython link that Cargo's test harness requires, so both stable and MSRV test jobs aborted with unresolved Python symbols before running a test. The workspace now enables only the Python 3.13 stable ABI, and Rust CI selects Python 3.13 before compiling it. Each Maturin project still enables `extension-module` when it builds a wheel.
- **The `RemoteClient` test double compiles under Rust 1.98's clippy** (`panproto-git-remote`): `clippy::unused_async_trait_impl` is new in 1.98 and rejects an `async fn` trait impl whose body never awaits, which is exactly the shape of a synchronous fake standing in for a network client. Rewriting it the way the lint suggests, `fn` returning `impl Future` around an `async move` block, trips `clippy::manual_async_fn` instead, so the two lints reject each other's output. The four methods now run their synchronous bodies and hand back `std::future::ready`, which neither lint objects to. They are always awaited immediately, so running the body at call time rather than on first poll does not change what the tests observe.
- **The CLI strategy-help contract is checked on both stable Rust and the MSRV** (`panproto-cli`): the assertion compared the tier descriptions as unformatted text, but Rust 1.85 preserved the Markdown backticks around strategy names. The test now removes those formatting marks before checking the names and tier membership; the generated-reference gate still checks the rendered help.
- **Schema-vertex transforms no longer spread ATProto terminology into generic crates** (`panproto-gat`, `panproto-lens`): the new exact-vertex operation called its protocol-neutral namespace field `nsid`, and diff conversion read that ATProto-specific field directly. The transform now carries a `namespace`, while the schema adapter contains the compatibility mapping to the existing schema record.
- **`panproto-c` is published and semver-checked** (`.github/workflows/ci.yml`): the crate had never reached crates.io. The release workflow authenticates with Trusted Publishing, which crates.io does not permit for creating a crate that does not yet exist, only for pushing versions of one that does. The workflow detects that case, warns, and continues, so every release since 0.41.0 published every other crate and skipped this one while reporting success. It is now on crates.io at 0.71.0, published once with an API token, and comes off the semver-checks exclusion list; it passes its baseline with 196 of 196 checks.
...
#### Substitution and renaming that stopped one level short

- **A binder no longer captures the term substituted under it** (`panproto-gat`, `panproto-expr`): `Term::substitute` alpha-renamed a `Let` binder when the replacement mentioned it and did not do the same for a `Case` branch binder, so substituting `y` for `x` in `case s of c(y) => pair(x, y)` produced `pair(y, y)`, two different values spelled as one. The expression language had the same gap at `Let` and `Match`, and its `fresh_name` avoided only the replacement's free variables and not the body's. Normalization runs substitution, and morphism equation-checking runs normalization, so a captured variable decided whether two theories were judged the same.
- **`free_vars` reports what is actually free** (`panproto-expr`): the scope bookkeeping removed a binder from the bound set on the wrong condition, so a binder leaked into its siblings and free occurrences there went unreported, while a shadowed outer binding was dropped and its bound occurrences were reported free. `record { f = \x -> x, g = x }` answered without `x`, and `\x -> (\x -> x) x` answered with it. A binder is now removed by whichever frame inserted it.
- **A rename reaches the terms inside a dependent sort's arguments** (`panproto-gat`): the colimit, the quotient, and the schema functor rewrote only the head of a sort expression. A dependent sort's arguments are terms, so an operation the construction renamed or identified kept its old name wherever it appeared inside one: a pushout identifying two points left `loop2 : Loop(pt2())` naming an operation the pushout does not declare, so the inclusion was not a morphism and the construction was not a cocone; the quotient left the same dangling reference. Every position that can name an operation now goes through `SortExpr::apply_maps`. Two related narrowings go with it: same-name declarations arriving from either side were compared before the second was rewritten, so declarations agreeing exactly once the identifications are applied were rejected as conflicting, and the quotient's compatibility check ran before the operation identifications were known.
- **A renamed sort's own parameters follow the rename** (`panproto-gat`): a self-dependent sort such as `Slice(of: Slice)` came out of a rename as `Segment(of: Slice)`, pointing at a name the theory no longer declares. Merging two sorts had the same gap for the kept sort's parameters.
- **Composing theory morphisms expands a derived-term assignment** (`panproto-gat`): a morphism may send an operation to a derived term rather than to a single operation, and composition rewrote such a term through the second morphism's operation-rename view, which by construction omits exactly the derived-term entries. An operation the second morphism expands therefore survived in the composite under its intermediate-theory name, and the composite was not a morphism. Composition now pushes a derived term through by full substitution.

#### Iteration order deciding a semantic outcome

- **A schema, an instance and a complement each have one encoding** (`panproto-schema`, `panproto-inst`): a schema's vertices, edges, hyper-edges, constraints, indices and enrichment tables, and an instance's node table, parent and children indices, labeled fan children, extra fields and annotations, are `HashMap`s and `HashSet`s and were written in whatever order the collection enumerated them. That order is decided by the process's hash seed, so one and the same value serialized to different bytes in every process: a committed data set is content-addressed, so the same data committed twice read as a change and the store filled with duplicates of one value. Every such table is now written in key order, at both levels where a map holds maps. Reading is unchanged. The regression tests write each fixture in 32 and 48 separate processes, each with its own hash seed, and compare the bytes; a single run would prove nothing here.
- **Every field of a node and a complement is written, so both read back** (`panproto-inst`): the version-control store holds a committed data set as MessagePack, which writes a struct as an array of its fields in declaration order. A node's position, shape and annotations, and six of a complement's tables, were omitted when they held their default, and omitting a field there does not mark it absent: it shifts every later field one slot left, so the decoder reads each of them as the wrong thing and the whole value fails to parse. A node carrying an annotation, or a complement recording its arc edges, could be committed and then never read back.
- **A migration ID is a function of the migration** (`panproto-vcs`): the canonical form keyed `hyper_resolver` by hyper-edge ID and discarded the label set that is the other half of the key, so entries sharing a hyper-edge collapsed onto whichever the bucket order delivered last and the same migration hashed differently in different processes (64 processes gave 2 different object IDs), while two migrations governing different label sets on one hyper-edge canonicalized to the same bytes and collided. The canonical key is now the pair, with the label component sorted and deduplicated.
- **One hyper-edge carries one resolver entry per fan shape** (`panproto-inst`, `panproto-mig`): a migration keys `hyper_resolver` by `(hyper_edge_id, labels)` so one hyper-edge can retarget differently depending on which children a fan carries, and compilation flattened that to the hyper-edge ID alone, discarding every entry but one, and which one survived was whatever the source table's bucket order delivered last. The compiled table now carries the full key, label sets are canonical, and two entries that canonicalize alike but disagree on their retarget are rejected at compile time rather than resolved by hash order. Fan reconstruction dispatches on the shape it is looking at, in both directions of the edit lens.
- **A lexicon project partitions the same way every time** (`panproto-protocols`): a `ref` pointing outside the parsed set has no owning document, so the placeholder stays in a file that references it, and where several documents referenced one target the ownership followed whichever edge the monolith's edge table produced first. Identical input therefore partitioned differently run to run (48 processes gave 4 different partitions) and every re-parse showed up as a version-control diff. Ownership is now decided on a total order: target name, then the referencing document's position in the input, first claim wins.
- **A theory's dependencies are derived from the schema, not from its layout** (`panproto-mig`): `dependencies_from_theory` needs a vertex to stand for each sort an equation mentions and took the first match out of the vertex table, which is the process's hash seed; one retraction law against a schema with four vertices of each kind gave 16 different dependency lists across 48 processes. A sort is now represented by the least vertex of its kind, and the derived list is ordered by the pair each dependency relates.

#### Constructions that claimed a law and then narrowed it

- **Foreign keys point at the right rows when tables merge** (`panproto-inst`): when a migration maps several source vertices onto one target vertex the target table is the concatenation of the source tables, and restrict copied foreign-key row indices across unchanged, so every pair silently addressed rows in whichever source block happened to land first. Pairs from distinct source edges identified by the edge map overwrote each other instead of being unioned, and both restrict and extend walked hash maps to decide the concatenation order, so the corruption varied run to run. Restrict now records each source block's offset and remaps every pair into it, and both operations visit sources in name order.
- **An empty factor empties a Pi product** (`panproto-inst`): the right Kan extension assembled each target vertex's table only from the fiber components whose source table had rows, so a two-element fiber with one empty factor degenerated into the other factor's table: rows the limit does not contain, handed back as the limit. Every fiber vertex is now a factor, an empty or absent factor empties the product, the empty fiber is the terminal one-row table, and colliding edges union their pairs.
- **An arc that contradicts the anchors it connects is rejected** (`panproto-inst`): validation checked only that an arc's edge was declared, never that the edge's endpoints agreed with the anchors of the nodes the arc joins, so a tree routing a `name` arc into a node anchored elsewhere validated clean, including through the C ABI. The endpoint check now runs alongside the edge-existence check and resolves the three indirections that make a mismatch legitimate: a union vertex stands for its coproduct variants, a recursion point for the vertex it unfolds to, and a reference vertex for the target of its outgoing `ref` edge. The W-type theory carried an empty equation list and now states the two coherence axioms it depends on.
- **A schema pushout quotients by the equivalence its overlap generates** (`panproto-schema`): the overlap's pairs are a relation and the pushout read them as a function from right vertex to left vertex, so pairing two left vertices with one right vertex overwrote the first pair and the square stopped commuting. Identifying two edges was worse: the endpoint identification it entails was never made, so the right injection sent an edge to an image running between vertices that were not the images of its endpoints, which is not a graph homomorphism. The overlap is now closed under a union-find over both vertex sets, taking in the vertex pairs and the endpoints of every edge pair, and an overlap naming an edge its schema does not contain is rejected with `SchemaError::OverlapEdgeNotFound`.
- **A composite migration carries each step's value action** (`panproto-mig`): the coercion that rewrites values at a kind-changing vertex was never part of the migration; `compile` looked it up from the kind pair its two schemas named, which only works while both schemas are at hand. Composing two kind-changing steps produces a migration between the outer two schemas, neither of which registers the coercion the intermediate schema did, so the composite compiled to no value action at all and emitted values still typed for the schema they came from. The data-level law `lift(g . f) = lift(g) . lift(f)` failed silently on exactly the migrations it matters most for. A migration now carries its own value action, `compose` composes the two steps' actions term by term, and `compile` prefers what the migration carries over what it can infer.
- **An inverted migration turns its value action around with it** (`panproto-mig`): inverting a step that had coerced an integer to a string gave back a migration that put the vertex's kind back and left the values as strings, so the structure travelled home and the data did not. The inverse now carries the reversed coercion keyed at the vertex the original sent it to, with the round-trip class turned around with it, and a coercion that records no inverse term is refused with `InvertError::CoercionNotInvertible` rather than producing a migration that looks total and is not.
- **A node relabelled out of the view keeps its payload** (`panproto-lens`): `RelabelNode` onto a non-surviving anchor returned the view delete and never added the node to `complement.dropped_nodes`, so the payload was lost from both sides and the HPW complement-coherence law failed. The pipeline now tracks the source tree the edits land on, so a node leaving the view arrives in the complement with the payload and the arc it had there.
- **`put_edit` keeps the complement in step** (`panproto-lens`): the backward direction never touched the complement, though the module's own first paragraph calls the complement a state machine that reacts to view edits. A view that deleted a node left the complement holding that node and its children, and a view that reused an id left the old record under it, so the next `put` restored stale content on top of the new node. It now clears the records for a deleted node and everything that reached the source only through it, and follows a relabel, a field edit and a fan delete into the complement.
- **A symmetric lens is held to a real middle** (`panproto-lens`): `SymmetricLens::from_span` checked the protocol, the vertex count and the vertex names of its two legs' source schemas and called that a shared middle, so two schemas agreeing on names while disagreeing on edges, kinds, constraints, requiredness, variants, orderings or recursion points passed. Synchronization puts a view back through one leg and reads the result through the other, so a middle instance built to one schema was then read against the other, losing arcs on every sync. The check is now over everything the legs traverse and reports the first disagreement as `LensError::NoSharedMiddle`.
- **`derive_theory!` states the axioms it declares** (`panproto-gat-macros`): the macro parsed the `axiom name: lhs = rhs;` items in its body and then built the theory with an empty equation list, so a theory introduced that way was lawless and instances violating its intended laws validated clean. The same body through `class!` carried the equations, which is what made the omission invisible. Both macros now expand their body through one function.
- **Existence checking stops rejecting recursive schemas and coerced kind changes** (`panproto-mig`): reachability inferred its roots from in-degree alone, so in a recursive schema, where the entry vertex is also a recursion target, no vertex has in-degree zero, the search started nowhere, and every surviving vertex came back a reachability risk, the identity migration included. Roots are now the schema's declared entries, falling back to in-degree zero only when a schema declares none. Kind consistency compared the two vertex kinds and stopped there, though compilation turns a registered coercion for that kind pair into the value transform the lift applies, so a migration whose lift is defined and total was reported inconsistent; the check now consults the coercion table as compilation does.
- **Both halves of the migration adjunction read the same domain** (`panproto-inst`): a compiled migration states a vertex's image either by naming it or by letting the vertex survive under its own name, and the pushforward read both while the pullback read only the first. A migration that renamed one vertex and let another survive pushed both forward and reindexed only one back, so the unit had no table to land the survivor's rows in, and the W-type pullback reported a node anchored at a surviving vertex as lying outside the image of a migration that had just carried it forward. A table at a vertex the migration says nothing about is now refused with `AdjunctionError::VertexOutsideDomain` rather than narrowed.
- **A merge says which same-name additions it identified** (`panproto-vcs`, `panproto-cli`, `panproto-py`): two branches that independently add the same name got one element in the merged schema and no record of it anywhere, which is a quotient of the pushout the merge claims to compute. The identification stays, since a schema is addressed by name and conflicting on it would leave a caller nothing to resolve, but the merge now reports it: `MergeResult::identified_additions` lists each identified vertex and edge in a sorted, process-independent order, `schema merge` prints them, and both the module docs and the book's merge sections describe the merge as the pushout quotiented by same-name identification rather than the free pushout.
- **A free model's fibers are keyed by the terms the theory identifies** (`panproto-gat`): term generation filed each closed term under its instantiated output sort as written, so under an axiom relating two index terms such as `pt1() = pt2()`, `Fib(pt1())` and `Fib(pt2())` were one fiber but two keys and an operation declared over the second found nothing in the first. Its applications were absent from the model, and because nothing new appeared at the final depth the model still reported `is_complete: true`, a certificate of initiality for a model missing elements the theory requires. Fibers are now keyed by their index terms' representatives under the theory's ground congruence, with generation and quotienting run to a joint fixpoint so the certificate stays honest. A theory with no indexed fibers or no equations pays nothing.

#### Unicode and byte offsets at parser boundaries

- **DDL keywords are matched in place instead of on an uppercased copy** (`panproto-protocols`): the SQL, CQL and Cypher parsers located keywords with `find()` on a `to_uppercase()` copy and then used the resulting byte offset to slice the original. Case mapping is not length-preserving, so the offset drifts: it cuts the wrong substring, or lands inside a multi-byte character and panics, both reachable from the public `parse_ddl`, `parse_cql` and `parse_cypher_schema`. A new `database::keyword` module matches the always-ASCII keywords case-insensitively against the original bytes. `name_after_keyword` also anchors the `IF [NOT] EXISTS` guard to the keyword rather than accepting the phrase anywhere in the statement, which previously let a string literal containing it hijack the table name; table-level `FOREIGN KEY` column names are no longer read out of the uppercased copy and lowercased back; and `DEFAULT` values keep their original case.
- **Terms and sort strings are scanned by character, not by byte** (`panproto-theory-dsl`): `find_top_level_keyword` walked raw bytes and sliced at every byte offset, so a term carrying a non-ASCII character sliced inside that character and panicked, reachable from the public `parse_term` and `parse_sort_expr`. `rewrite_sort_string` had the mirror defect in the other direction, pushing each UTF-8 byte as a Latin-1 character so any non-ASCII text came out mojibake. Both scans now iterate `char_indices()`.
- **A JSON string's surrogate pairs survive extraction** (`panproto-io`): `\uXXXX` escapes were decoded one at a time, and a surrogate pair is the only way JSON can spell an astral character, so neither half was a character, `char::from_u32` rejected both, and `{"a":"x😀y"}` read back as `xy`. The segments' raw texts are now concatenated and decoded as one body, with a high surrogate pairing with a following low surrogate; a surrogate with no partner keeps its escape text rather than being dropped.
- **A non-UTF-8 delimited cell surfaces as bytes instead of lossy text** (`panproto-io`): the byte-faithful delimited codec decoded every field with `String::from_utf8_lossy` and then decided "changed" by comparing the re-encoded bytes against the original, so for a latin-1 payload, the EDI X12 and SWIFT MT case the codec exists to serve, the comparison always reported a change and emission replaced each non-UTF-8 byte with `U+FFFD`. An unmodified round-trip destroyed the file, against the codec's byte-faithful contract. A field that is not valid UTF-8 now surfaces as `Value::Bytes`, which the splice path already replays verbatim.

#### Format-preserving emission

- **Growing a value no longer deletes the rest of the file** (`panproto-parse`): replay decided which fragments the output had already passed by advancing a cursor by each fragment's current text length from its original start byte, and those are the same number only while nothing has been edited. Once injection rewrote a literal to something longer than the bytes it replaced, the cursor ran past the recorded start of every following fragment and each was silently dropped: editing one value truncated everything after it. Coverage is now a question about original spans alone, and the rewritten text decides only which bytes come out, never which fragments do. Replay's fragment order is also made total, so a tie between a leaf's literal and the interstitial recording the same bytes no longer resolves by constraint-map iteration order.
- **An edited string addresses its whole segment run** (`panproto-io`): tree-sitter splits a JSON string body into alternating `string_content` and `escape_sequence` children, so `"x\ny"` arrives as three vertices and the complement recorded only the first. Injection wrote the whole re-encoded value into that first vertex and left the others to replay, so editing `"x\ny"` produced `"x\ny-perturbed\ny"`, the old tail appended to the new value. A node id now maps to the ordered run of vertices the value occupies; injection writes into the run's head and empties the rest, which is coherent because every vertex carries the source span it covers.
- **An open-schema YAML document extracts the values it holds** (`panproto-io`): reading a document whose shape is not known in advance is the ordinary case and the JSON extractor handles it by synthesising one edge per child, while the YAML extractor did not: a sequence reaching a domain vertex with no item edge collected nothing and a mapping folded its pairs into `extra_fields`, so an open-schema YAML document parsed to a bare root and nothing downstream could address a value in it. Both extractors now fall back the way the JSON ones do. Fixing that surfaced why the extractor looked shallow: the grammar wraps every mapping value, sequence element and document body in a `flow_node` or `block_node`, and puts a scalar's text one level below the `plain_scalar` the dispatch reaches. Injection gains the guard the JSON path already had, since a YAML scalar's lexical form is not recoverable from the parsed value.
- **TOML is read through TOML's own vocabulary** (`panproto-io`): the TOML codec routed its CST to the JSON extractor, which dispatches on JSON's node kinds, and a TOML tree carries none of them, so every document extracted to one empty root node. The unmodified round trip still looked byte-faithful because nothing had been extracted and so nothing could be written back, and the canonical emitter turned the same document into JSON. TOML now has its own extractor and injector reading the shape TOML actually has, and canonical emission writes TOML through the `toml` crate rather than hand-assembling text.
- **A tabular cell is addressed by a key that cannot collide** (`panproto-io`): the layout complement packed a cell's row and column into a u32 as `row * 10_000 + column`, which is not injective, so row 1 column 0 and row 0 column 10_000 shared one entry and an edit to either was written into whichever CST vertex won the insert; it also overflowed the u32 outright at around four hundred thousand rows. A wide feature matrix and a long log export both reach those numbers. Cells now key on a u64 with each coordinate in its own half, in a map of their own rather than the one that addresses W-instance nodes by id.
- **Deep nesting is an error rather than an abort** (`panproto-parse`, `panproto-io`, `panproto-theory-dsl`): every public parse entry point, the JSON, XML and YAML CST extractors, and the theory DSL's recursive-descent `parse_term` walked their input with no bound, so nesting deep enough to exhaust the thread's stack killed the process on a signal no caller can handle. A parser reads bytes it did not author, so this has to be an error value. Each now carries an explicit depth bound and returns `ParseError::NestingTooDeep`, `CstExtractError::NestingTooDeep`, or the theory DSL's depth error. The walker also threaded its schema builder by value through a fluent API, so an unoptimised frame cost about forty kilobytes and three dozen levels was enough to die; the builder now lives in a cell passed by reference and a frame costs about four and a half kilobytes, taking the depth the walk reaches from 32 to 416. Regression tests run at exactly each limit, so a change that grows a frame fails a test rather than surfacing as an abort in the field.
- **A mutation round-trip corpus covers the modify path** (`panproto-io`): the unmodified round trip was proven by the existing corpus and the modify path had almost no coverage, which is where the four corruption bugs above lived. For every fixture and every scalar it carries, the corpus parses, perturbs exactly one value, emits, and reparses, asserting the result is well formed and AST-equal to the edit, across JSON, XML, YAML, TOML, CSV and TSV. Perturbations grow the token, since a shrinking edit leaves slack that a length-based emitter absorbs.

#### Expression evaluation

- **Graph builtins answer wherever they appear in an expression** (`panproto-inst`, `panproto-expr`): instance-aware evaluation matched on the root of the expression, so a graph builtin at the top was resolved against the instance and anything else was handed to the pure evaluator, which returned `null`. `edge_count self > 0`, the shape a survival predicate or a computed field actually takes, therefore compared `null` against zero, and the same silence applied under a binding, a lambda or a comprehension. Misapplying one was worse: the handler read `args[0]` and `args[1]` without checking how many it was given, so `edge 1` took the process down. The evaluator now takes a `BuiltinResolver` consulted at every point a builtin is applied, each graph builtin checks its arity first, and the pure evaluator reports `ExprError::NoInstanceContext` rather than answering `null`, since `null` made "no such edge" and "no graph was consulted" the same answer.
- **The one overflowing remainder is reported instead of aborting** (`panproto-expr`): every integer builtin but `mod` went through a checked operation, and `i64::MIN % -1` overflows unconditionally in Rust, in a release build as much as a debug one, so an expression reaching that pair took the whole process down. `mod` now takes `checked_rem`, and integer overflow in the shared binary-operator path is reported as `ExprError::Overflow` rather than as a type error claiming the arguments were the wrong type.

#### Error handling and structure

- **A failed lift carries the chase's own error** (`panproto-mig`): `LiftError::Chase` flattened the chase's error to a sentence, so a budget that may converge on a larger allowance and an equality conflict that recurs at every budget arrived indistinguishable and a caller wanting to retry had to match on the text. It now carries `ChaseError` itself, a term-level chase's budget exhaustion becomes `LiftError::ChaseBudgetExhausted` naming both ceilings, and `is_retryable` answers the question directly.
- **The lens DSL refuses malformed terms and defaults and builds expressions as trees** (`panproto-lens-dsl`): `parse_term` was infallible, so `"1 + 2"` and `"a b"` became variables whose names are the whole string, `"f(a, b"` closed its own argument list at the end of the input, and `"f(a))"` dropped everything after the closing parenthesis, each producing an equation the document never wrote. `json_to_value` narrowed anything it could not represent, turning an array or object default into `Int(0)`, `Float(0.0)` or `""` and an integer past `i64::MAX` into `Int(0)`. And the rules compiler built expression source by interpolating user text into a format string, so a quote or a backslash in a template segment or a string operand changed what the generated source meant and a field name with a space referred to something else entirely. The grammar is now enforced, arrays and objects become `Value::List` and `Value::Unknown`, and both builders construct the expression tree, with steps carrying `ExprSource`.
- **An unsound rewrite system is refused rather than warned about** (`panproto-theory-dsl`, `panproto-protocols`): the gate ran and threw its answer away, discarding the analysis error and printing each non-joining critical pair and LPO violation to stderr from library code, then handing back the theory as though it had passed. Normalization decides a theory's equality judgment by running that rewrite system, so a caller who never reads stderr, or who is a library, a wheel, or a WASM module with no stderr to read, went on to normalize against an unsound judgment. `compile_theory` now returns `TheoryDslError::UnsoundRewriteSystem`, and `panproto-protocols` panics as its colimits already do, since the theories it gates are assembled from its own building blocks. Turning the gate on surfaced an unsound fixture in the crate's own tests, a rule `x -> x` that never makes progress.
- **Each lexicon file gets only its own structure** (`panproto-protocols`): splitting a bundle into per-file schemas filtered the vertices, edges, constraints and the rest down to the owning file, then copied the bundle's hyper-edges, spans, coercions and policies wholesale into every file, so a ten-document bundle produced ten copies of each and each file claimed hyper-edges over vertices it does not have. A hyper-edge now goes to a file only when every vertex it names is that file's, a span only when both endpoints are, and coercions and policies only when the kinds keying them are kinds the file's vertices carry.
- **A backward migration refuses a complement list of the wrong length** (`panproto-c`, `panproto-wasm`): both boundaries paired each migrated view with its complement through `.zip()`, which stops at the shorter list, so three records and two complements restored two and reported `record_count: 2` and the dropped record left no trace anywhere in the result. The lengths are now checked before any `lens::put` runs. `migrate_dataset_forward` in the WASM port also leaked a slab slot when complement serialization failed, which the C port had already fixed; both carriers are now built before either allocation.
- **An FFI error is attributed to the process, not the calling thread** (`panproto-c`): the last-error slot was a `thread_local!`, so an entry point that failed on one OS thread left its envelope where `pp_last_error_take` on another could not see it. That contradicts the slab's own rationale, since GHC's threaded RTS migrates a Haskell thread across OS threads at every `safe` foreign call and the Haskell binding drains the slot through a separate call, so a failure and its drain routinely land on different threads. The slot is now a process-global mutex-guarded static mirroring the slab. `PpStatus` also documents each variant instead of suppressing `missing_docs`.
- **A composed resolver entry speaks the source label space** (`panproto-mig`): `compose(m1, m2)` is a migration read against `G1` data, and adopting an entry from `m2` left both halves of it in `G2`, so the entry selected on labels no source fan carries and renamed labels that do not exist. Both halves are now pulled back through `m1`'s label renaming and vertex map, with the least of several source labels standing for its class and entries adopted in key order.
- **The optic and lens law checks name the law they check** (`panproto-lens`): `check_optic_laws` reported the two lens laws under each other's names, so a failing optic named the law it did not break, and `check_put_put` called PutPut the well-behaved-lens law when it is the law separating a very well-behaved lens from a well-behaved one.

### Performance

- **Binding a name costs the same at any scope width** (`panproto-expr`): every `let`, lambda and closure application extended the environment by cloning the whole map, and a lambda additionally copied every binding in scope into a `Vec` to build its closure, so binding one name cost as much as the scope was wide. The environment is now a shared chain of scopes and a closure carries the scope itself rather than a snapshot. Iteration and equality read the chain in name order with a shadowed name yielded once, which is what makes a closure's identity, its hash, and the canonical encoding taken over it a function of what it binds rather than of the order it was built in. Measured on unused bindings, the cost ratio between a 200-wide and a 3200-wide scope falls from 8.9 to 0.8.
- **Tokenizing walks the source once** (`panproto-expr-parser`): the layout pass computed each token's line and column by scanning the input from its start, which makes the pass quadratic in the source's length. Tokens reach the pass in ascending offset order, so a cursor carrying the current line and the offset its line began at answers both questions and crosses each byte exactly once. Measured on one binding per line, 500 lines took 315.9ms and 2000 lines took 5.96s, a ratio of 18.9; the crate's whole suite now runs in 1.2 seconds where the timing test alone took 30.
- **A wide node is walked and replayed in linear time** (`panproto-parse`): the walk resolved a child's tree-sitter field name by asking its parent for children one index at a time, rescanning the whole child list once per child, and replay resolved each interstitial run's recorded span by scanning the whole constraint list of the vertex carrying it. A wide node is ordinary data: a long array literal, a spreadsheet row with thousands of columns. The parent now resolves each field name from the index it is already iterating, and constraints are indexed by sort once per vertex. A 20,000-element array literal did not finish inside ten minutes before and takes 2.1 seconds now.
- **A schema lookup through the slab costs a refcount bump** (`panproto-c`): the slab stores a schema behind an `Arc` precisely so a lookup can be cheap, and twenty-five call sites reached for it through `as_schema()` and then deep-copied every vertex, edge, span, coercion and policy map. `Resource::as_schema_arc` hands the slot's `Arc` out instead. Measured with a 20,000-entry schema, 200 lookups fall from 521.4ms to 280.9µs. The four enriched-schema entry points keep the deep copy, since they mutate the schema they fetch. No exported signature, struct or status code changes.
- **Long native work releases the interpreter lock** (`panproto-py`): every native operation ran with the GIL held, so a Python program that farmed parsing, project assembly, lens generation or a git import out to a thread pool got no parallelism at all, and any unrelated Python thread was starved for the duration of each call. The operations whose cost scales with the input now run inside `Python::detach`. Measured over four parses of a 170 KB source, the threaded speedup goes from 0.88x to 2.84x, and interpreter iterations during one parse from 58,286 to 328,404,860. No signature or return type visible from Python changed.
- **Row-existence saturation stops copying the pattern table** (`panproto-mig`): saturation copied a dependency's whole pattern table on every pass for every dependency, only so the borrow would end before the consequence table was touched. The loop over matching rows was deciding one thing, whether any row matched, and reading that answer out ends the borrow just as the copy did. Measured on a 4,000-row table read by 200 dependencies, 1.510s falls to 1.485ms.
- **Resolving a term in a free model uses the index already built** (`panproto-gat`): `find_term_index` scanned every generated term of every sort to resolve one term, and the equation pass calls it twice per substitution instance, so a two-variable axiom cost a full traversal per ordered pair of terms. It now shares the lookup table `build_congruence_index` was already building. Measured on a three-constant binary theory at depth 2, 1.331s falls to 267.6ms.
- **Composing two lenses collects one set** (`panproto-lens`): composition walked all of the first migration's vertex remap once per entry of the second's.

### Documentation

- **The book and every maintained README now describe the implementation directly**: the prose pass removes scene-setting and promotional language, shortens overloaded sentences and defines mathematical terms before using them. The migration chapters now distinguish restriction from the three data-migration functors, define the apex and legs of a span, describe the implemented search algorithms and bounds, and state that `Llm` is a reserved alignment-evidence tag rather than evidence produced by a configured language-model strategy. CLI, SDK, crate and grammar documentation has been checked against the current entry points and wire formats. The bibliography now credits the original work behind bucket elimination, hybrid best-first search, EDAC*, all-different propagation, McSplit, lenses, functorial migration, institutions, colimits, Tree-sitter and the alignment methods discussed in the book.

- **The facade, the theory inventory and the square check describe what the code does** (`panproto-core`, `panproto-protocols`, `panproto-vcs`): the `panproto-core` facade said the four binding crates depend on it alone, and two of them do not, since `panproto-cli` and `panproto-py` take `panproto-parse`, `panproto-project` and `panproto-git` directly rather than through the feature-gated re-exports, and the CLI also takes `panproto-xrpc`, which the facade does not re-export at all. The `panproto-protocols` theory module said it held five building blocks while exporting eleven, with the six it omitted filed under a heading calling them private helpers. And `panproto-vcs::square` called its check the double-category square condition when what it checks is that migrating a data set forward and back through its complement reconstructs it, the vertical arrow's `GetPut` law; the module now says which direction the check is decisive in, since a divergence rules the square out and agreement does not establish it.
- **`functor_restrict` is documented as the pushforward it computes** (`panproto-inst`): it was documented as `Delta_F`, precomposition, in the module header, the function, the shared `AcsetOps::restrict` contract, `lift_functor`, the value-action docs on `TermAssignment`, a command-line error message, and the name of an integration test. Given a migration `F: S -> T` it takes an `S`-instance and returns a `T`-instance, and where the vertex map merges two vertices it concatenates their tables into the image's, which is what `Sigma_F` does; precomposition runs the other way and lives in `adjunction::f_delta`. The docs now say what each of the two operations does and how they differ.
...
- **Their two stale references** (`.github/workflows/ci.yml`, `tests/integration/tests/genericity.rs`): the `cargo-semver-checks` exclusion list named both crates, and `LANGUAGE_ID_EXEMPT_CRATES` exempted `panproto-llvm` from the language-identifier scan. Neither could match a workspace member any more. The list is now empty: every publishable workspace member is under the gate.

## [0.71.0] - 2026-08-18

### Security

- **`h2` moves from 0.4.13 to 0.4.16 for RUSTSEC-2026-0258**: the crate accepted and queued empty `DATA` frames without limit, so a stream that was not actively drained could grow memory without bound, or panic if the length overflowed. It reaches this workspace transitively, through `panproto-xrpc` to `reqwest` to `hyper`, so the shipped CLI binaries are affected; the Python wheels and the npm package do not depend on `panproto-xrpc` and are not. `publish-crates.yml` publishes with `--locked`, so the lockfile is what governs a released artifact.

### Added

- **`panproto_schema::induce` and `induce_on_vertices`** (`panproto-schema`): the supported way to cut a sub-schema, accounting for all twenty-one `Schema` fields in their own key spaces and rebuilding the three adjacency indices rather than copying them. `induce_on_vertices` takes a vertex set and keeps every edge between survivors; `induce` takes both sets. Both validate the result against the protocol and report `SchemaError::InducedSchemaInvalid` carrying the findings, which is what makes a span's apex a schema rather than a fragment.
- **`panproto_schema::canonical_bytes` and `canonical_digest`** (`panproto-schema`): a stable content identity for a `Schema`, and one that deliberately disagrees with the VCS content hash it sits beside. It covers all twenty-one fields, including `entries` and the three adjacency indices, and writes every `Vec` in stored order, where `panproto_vcs::hash::CanonicalSchema` covers seventeen and normalises. Two schemas equal as values hash equal; a schema and its re-indexed twin do not, which is the property the span search needs and the VCS does not.
- **`pp_hom_find_span` and `pp_hom_span_to_overlap`** (`panproto-c`): the span search and its projection to the pair lists a pushout takes, so a non-Rust host reaching for what two schemas share no longer has to go through the total-morphism search and accept its refusal. The `hom` section of the ABI goes from five exports to seven and stops being a Python-only surface. `pp_hom_find_span` takes a `Protocol` handle the older entry points did not need, because the apex is re-validated against a protocol the source schema only names.
- **`auto_generate_span`** (`panproto-wasm`, `@panproto/core`): a WASM export and the `Panproto.span(from, to, hints?)` and `ProtolensChainHandle.autoGenerateSpan(...)` calls over it, answering with a nine-key `SpanResponse` where `Panproto.lens` and `Panproto.protolensChain` threw "no morphism found between schemas". Two schemas with nothing in common come back as an empty apex with `apex_coverage` zero rather than as an exception, and `apex_digest` is a value a caller can key a cache on.
- **The span surface on Swift** (`panproto-swift`): `SchemaHandle.findSpan(to:in:options:constraints:)`, `SchemaSpan.overlap()`, and the `PanprotoFFI` shims under them. This is the call to reach for when `findBestMorphism(to:options:)` answers `nil`, and it takes a `ProtocolHandle` the older search did not.
- **Domain restrictions and objective weights are a payload a host can send** (`panproto-swift`, `panproto-haskell`): Swift gains `MorphismDomainConstraints` and `MorphismCostWeights`, Haskell gains `DomainConstraints`, `CostWeights`, their defaults and their codecs. A host can now pin where each source vertex may land, exclude sources and targets, and set the five weights of the objective, none of which the C boundary previously carried.
- **`FoundMorphism.edge_map`** (`panproto-py`): readable from Python for the first time, as `list[tuple[Edge, Edge]]`. `FoundMorphism.to_dict()` gains a third key alongside it, so a consumer comparing the dict's key set exactly sees a shape change.

### Fixed


- **An excluded source vertex is dropped from the answer rather than from the problem** (`panproto-mig`): `DomainConstraints::excluded_sources` removed the vertex from the variable set and skipped its incident edges, so a total-morphism search returned a `FoundMorphism` whose `vertex_map` covered only the induced sub-schema while claiming to be total. Exclusion now forces `⊥`, and a morphism is rejected unless it maps every source vertex, so the same request answers empty. A caller wanting the sub-schema answer asks for a span.
- **`quality` is read out of the objective the search minimised** (`panproto-mig`): the number was accumulated by a separate `f64` scorer that could disagree with what the search optimised. It now comes from the decomposed fixed-point objective, and the Jaccard component's normaliser is `C_src`, the source vertices carrying at least one named outgoing edge, rather than a count that varied with the assignment. For a fixed morphism the reported score differs wherever the two normalisers do: a source leaf mapped onto a childless target drops out of the denominator instead of scoring zero in it, which raises the mean. Any threshold calibrated against the old number needs re-reading.
- **An inverted migration carries its endpoints** (`panproto-mig`): `invert` set both `domain` and `codomain` to `None`, and `compose` skips its composability check when either side is `None`, so an inverted migration composed with anything. It now carries the original's endpoints swapped, and a composition that does not line up is refused.
- **A merged or normalized schema's adjacency order is a function of its inputs** (`panproto-schema`): `schema_pushout` and `normalize` both rebuilt the three adjacency indices by iterating a `HashMap`, so `outgoing_edges`, `incoming_edges` and `edges_between` came back in a different order in every process. Both now build in an order derived from the inputs, preserving each side's own sibling order, so a consumer reconstructing source text from a merged or normalized schema, or digesting one, gets the same answer twice.
- **`auto_generate` keeps the better of the pinned and released searches** (`panproto-lens`): it short-circuited as soon as the pinned search mapped every source vertex, so a released search that would have found a higher-quality alignment over the same vertices never ran. The two are now compared on the objective, quality first and coverage second. Measured over the corpus this changes the answer on 66 of 5852 pairs, recovering a median 0.0012 and as much as 0.3251 quality, at the cost of a second span search per call.
- **The lens no longer pre-excludes source vertices it thinks are infeasible** (`panproto-lens`): three scans populated `excluded_sources` before the search ran, at the more permissive tiers, on the theory that a vertex with no compatible target should be dropped early. The search already answers that question exactly, and the scans were stricter than it: a root whose only outgoing edge had no target counterpart was excluded along with the orphan leaf, so the generated chain carried a `DropSort` for a record the search would have kept. Fields that used to disappear at a span tier now survive.
- **The span is a function of the two schemas, and not of the order either was built in** (`panproto-mig`): `edge_image` chose among parallel target edges of one kind by taking the first the slice held, and `Schema::edges_between` returns its index in whatever order built the schema, so the right leg's edge map moved between two schemas that were equal as values and, for a schema a three-way merge assembled from a `HashMap`, between runs of one unchanged program. It now takes the least candidate by value. The apex hid this throughout: it is canonical and sorts everything it reads, so it stayed byte-identical while the leg moved underneath it, and any test comparing apex digests would have passed.
- **The maximum common sub-schema search reads the apex constraints instead of only checking them at the leaf** (`panproto-mig`): the constraints were enforced exactly, by scoring each candidate incumbent and refusing an infeasible one, and used nowhere else. The bound reads the objective and the capacity of a label class, and an apex hard constraint pays no reward, so on a source whose structure is annotation maps rather than arcs the bound is the bare sum of the per-vertex maxima and assumes every source vertex maps at once. A vertex tied by a forbidden entry to one already dropped can never be mapped, since a dropped vertex is `⊥` in every completion below that node, so it now leaves its class and the capacity falls with it; a mapped vertex whose partner has gone ends the node. On the nine-vertex pair a fuzzer found, whose eleven constraints admit one vertex, this takes the search from ten million nodes and sixty seconds returning the empty apex without a proof to 102 nodes and 1.2 milliseconds returning a strictly cheaper answer with one.
- **A recursion point that dangles at either end is reported by `validate`** (`panproto-schema`): inducing a sub-schema keeps a marker only when both its ends survive, so a marker naming a vertex the schema never had was dropped rather than reported and the resulting apex looked complete.
- **The Python type stub declares the protocol methods and return types the extension has** (`panproto-py`): `py.typed` makes the stub authoritative and a type checker never consults the extension, so `len(schema)` was an error on a call returning a number. `__len__` is implemented on four classes and was declared on one; `Repository.add` and `Repository.gc` return dictionaries and were declared as `None`. The parity test grew a pass over the callable dunders each class's own dictionary carries, which is the comparison that had been filtering `__dunder__` names out before making it.
- **A search that could not be posed is no longer reported as "no morphism exists"** (`panproto-mig`, `panproto-c`, `panproto-py`, `panproto-lens`): a variable's domain is a fixed-width bitset, so a source vertex offered more kind-compatible targets than it holds is refused. `find_morphisms` turned that refusal into an empty list and `find_best_morphism` into `None`, which their own contracts define as "no total morphism exists". A schema searched against *itself* was therefore told its identity morphism did not exist, and the same answer reached Python, the C ABI and the SDKs built on it under a success status. All four total-morphism entry points now return `Result`, so `Ok([])` and `Ok(None)` mean what they say and a capacity refusal is reported. The same laundering is fixed at every call site that repeated it: `auto_lens`'s search closure, the candidate path's morphism list, and the overlap fallback. The ceiling itself is removed under *Changed* below, so what these entry points now report is a measured memory cost rather than a word size.
- **`epic` is a constraint the search enforces, not a filter over its answer** (`panproto-mig`, and every binding): surjectivity was tested in `morphism_of` after the optimiser had already committed to the optimum of an objective that does not mention it, so `find_morphisms(epic: true)` returned empty whenever the optimum happened not to be onto, however many surjective total morphisms existed. It is now a leaf predicate inside branch and bound, so the search optimises over the surjective assignments and returns one whenever one exists. Two consequences follow. Capping the result count no longer changes whether an answer is found at all, which is what `find_best_morphism` did by forcing `max_results = 1`; and `find_span` now rejects the flag with `SpanError::EpicIsNotASpanProperty` instead of dropping it, because a span's right leg is deliberately partial and the entry point is documented never to refuse for want of a match. Cardinality decides the unsatisfiable cases before any search runs.
- **A migration that identifies two anchors is refused rather than lifted** (`panproto-inst`): `wtype_extend` had no counterpart to the guard `wtype_pi` applies, so a contracting vertex map produced a node carrying both subtrees side by side, which is two values under one field and not an instance of the target schema. `Sigma` accepted the same compiled migration `Pi` rejected, and nothing downstream noticed. Both variants now report `RestrictError::NonInjectiveVertexMap`; `Migration::resolver` is an edge disambiguation table rather than a merge rule, so there is nothing in the lift path that could have carried the merge out.
- **An arc whose edge a migration dropped is no longer relabelled onto a sibling's** (`panproto-inst`): the extension resolved such an arc from its remapped anchors, and that resolution returns whichever single target edge runs between them. A source with parallel `a` and `b` arcs migrating onto a target that kept only `a` therefore delivered `b`'s value under `a`'s name. The arc now keeps its own kind and name, taking the target edge that agrees with it, and is dropped when the target carries none: a dropped field is a loss, a relabelled one is a lie.
- **An auto-derived migration never contracts on edges either** (`panproto-vcs`): `SearchOptions::monic` promises injectivity on vertices and nothing more, so the right leg of a monic span could still send two parallel source edges to one target edge, and the stored commit object recorded a migration saying two fields become one without saying how. The derived edge map is now pruned to be injective, keeping the name-matched image and leaving the loser unmapped. Declining the whole leg instead would have been worse, since the fallback is the diff-derived migration, which loses the surviving field as well.
- **A kind-crossing rename is no longer pinned** (`panproto-vcs`): `detect_vertex_renames` scores matching edge labels and short edit distance without requiring the kinds to agree, so a 0.4-confidence detection could rename an integer field onto a string one. Pinned, that leaves the vertex with `⊥` as its only value and the search drops it, silently, even when a kind-compatible target was available. Pins now require kind agreement; a dropped pin strands nothing, because the vertex falls back to its whole domain. No threshold could have fixed this, since matching outgoing names alone already scores 0.5 with no kind credit.
- **`schema auto-migrate --total` no longer reports that no total morphism exists on a pair that has one** (`panproto-cli`): the rung ran the span search and rejected the answer unless it was total, but span quality excludes the drop count while the objective is lexicographic in `(quality, drops)`, so a span that drops a vertex can score strictly better than a total morphism that keeps it. An optimal partial span is therefore no evidence at all about existence. The rung now falls back to the total-morphism search, which answers the question actually asked, and reports what it finds; the fallback runs only where the command used to bail, so a pair whose optimal span is already total pays nothing.
- **`schema auto-migrate` says when its answer identifies two source vertices** (`panproto-cli`): a contracting right leg is an ordinary answer from the default search and is not a migration a lift can carry out, since both fields' data would arrive under the survivor's name. The certificate already recorded the fact and the command dropped it. It is now reported on stderr, which keeps stdout pipeable under `--json`.
- **A discovered overlap no longer identifies two apex arcs with one target arc** (`panproto-mig`): `bijective_edge_map` tested surjectivity against the *whole* target rather than against the target edges between mapped endpoints. On the isomorphism path the two tests agree; on `discover_overlap`'s path the apex is a maximum common induced sub-schema and is a proper part of the target for essentially every non-isomorphic pair, so a bijection that had just been built successfully was discarded and the greedy fallback sent both parallel arcs to the first arc of their kind. The pushout then kept one preimage and invented a fourth arc present in neither input. `LegShape` gains `right_edge_images`, which records the fact `right_leg`'s documentation already claimed the certificate carried.
- **`SchemaSpan::pushout` refuses a span whose right leg contracts** (`panproto-mig`): merging along the apex has to commute, and a right leg sending two apex vertices to one target vertex makes that impossible, since the merge identifies elements by a map keyed on the target element and a repeated key names only one preimage. The square that came back was not a cocone over the span it was asked about, and nothing said so. It now reports `SpanError::ContractingRightLeg`. `discover_overlap` already searched with `iso` for this reason, which is why the CLI's merge path was never affected.
- **A pushout keeps a vertex's NSID and the `nsids` map in step** (`panproto-schema`, `panproto-vcs`): a schema records each NSID twice, and the two merges disagreed about which side wins. `build_merged_vertices` merges whole vertices, so the left one wins outright, while `nsids` is merged per key and fills an absent left key from the right. Identifying a left vertex carrying no NSID with a right vertex carrying one therefore left the vertex saying `None` while the map said the NSID, a contradiction baked into the content address and one that made lexicon emission, which finds the root by the vertex's copy, lose the NSID entirely. `three_way_merge` had the same split for a different reason and is fixed with it.
- **A pushout no longer re-keys coercions and policies through the vertex rename map** (`panproto-schema`): a coercion is keyed by a pair of vertex *kinds* and a policy by a constraint sort name, and neither is a vertex id. A right vertex whose id happened to spell a kind or a sort dragged the key into the vertex namespace, where nothing could look it up and a later induction, which filters coercions by surviving kinds, deleted it for good. Both maps now merge on their own keys. The intended consequence is that a right coercion for a kind pair the left already has now loses to the left, which is the same left-wins rule the vertex-keyed maps already follow.
- **`normalize` keeps every policy** (`panproto-schema`): `policies` is keyed by constraint sort name and was filtered against the surviving *vertex* ids, so normalizing any schema with a ref vertex dropped every conflict-resolution policy it had. A ref vertex is how each of the schema languages spells an indirection, so this was the ordinary path rather than a corner. The map now carries over wholesale, which is what `induce` already did.
- **`induce` intersects `keep_v` with the vertices it kept, on every rule** (`panproto-schema`): the `# Selection` contract says ids naming no vertex are ignored, and only `vertices` and `edges` held to it. Ten further vertex-keyed fields tested the caller's request alone, so a parent holding a row filed under an id it has no vertex for passed that dangling reference into the apex whenever the caller named the id. `entries` was the worst of the set, since a ghost basepoint makes `entry_vertices` hand out a vertex the apex does not hold.
- **A strategy anchor no longer costs coverage the unpinned search would have had** (`panproto-lens`): alignment strategies are heuristics and their output is merged into `SearchOptions::hard_pins`, which is reserved for mappings a caller knows. A pin collapses its vertex's domain, so a pin that is individually plausible but jointly infeasible with the rest does not make the search fail: the vertex becomes unmappable and the optimum drops it, with an optimal span, a proven-optimal certificate, and the field simply missing from the lens. The retry that exists for this could only fire on `Err`, which a tier allowing spans never produces, since the search answers `⊥` rather than refusing. Both the single-result and the candidate entry points now compare the pinned attempt against the released one on what each covers, and take the released answer only when it covers more, so the pins keep the pairs they were winning.
...
- **A user hint again reads as the caller stated it** (`panproto-mig`): `adjust_anchors_by_required_sets` applied its `±0.05` tiebreak to every anchor in the pool, including `UserHint` ones. Because the hint reaches the aggregated score through a `max` against its own capped confidence, a hint stated at 1.0 across a schema change that made a field optional came back as 0.95, contradicting the documented behaviour that a hinted pair reads 1.0. Requiredness disagreeing across the two schemas is the normal consequence of the change being hinted through, so this was the likely case rather than a rare one. Hints are now exempt: a hint is a caller stating a correspondence, not a heuristic proposing one, so there is no tie for a tiebreak to settle.
- **`auto_symmetric` builds its middle schema by inducing it** (`panproto-lens`): it assembled the overlap by hand, selecting vertices and edges into a fresh `Schema` whose `outgoing`, `incoming` and `between` indices were left empty while its edge map was populated, so every adjacency query on the middle schema answered with nothing. It also silently dropped entries, required sets, variants and recursion points. It now takes the apex of one span search on the iso path, which is induced and therefore carries the non-edge structure and rebuilt indices.
- **The span certificate reports both legs' existence obligations** (`panproto-mig`): only the right leg's was computed, and the single `existence` field read as a statement about the span. The two legs have different codomains and can fail different obligations, so they are now `left_existence` and `right_existence`.
- **`is_total` no longer counts a source edge the apex cannot hold** (`panproto-mig`): `Schema`'s fields are public, so an edge can name a vertex the schema does not hold. Such an edge has no variable at that end and can never be induced into the apex, so comparing the apex's edge count against `src.edges.len()` made a span covering the whole source report as partial.
- **`schema auto-migrate` prints its edge map in a stable order** (`panproto-cli`): it iterated a `HashMap`, so one schema pair printed its edges in a different order on every run and the output could not be diffed against itself.
- **A span tier's candidate list comes from the span search** (`panproto-lens`, and every binding reaching `auto_generate_candidates`): `auto_generate_candidates` and `auto_generate_candidates_with_hints` routed every tier through `find_morphisms`, which now returns the *total* morphisms attaining the optimum. No total morphism exists whenever the source carries a sort the target lacks, which is the ordinary case on real schemas, so both entry points reported "no morphism found between schemas" on exactly the pairs a span tier exists for. `run_search` already read a span tier off the span search; the two candidate entry points now do the same, taking each optimal span's right leg and discarding the spans whose apex is empty. The single-result entry points were unaffected, which is why the corpus tests stayed green and the failure surfaced only through the bindings.

### Changed

- **A variable's domain is as wide as the schema pair needs it to be** (`panproto-mig`): a domain was one machine word, so a source vertex offered more than 63 kind-compatible targets could not be represented and the pair was refused. That is ordinary input: a record type with 64 fields of one type meets it, and the shipped `raw_file` protocol makes one vertex per line, so a 64-line file could not be searched against itself. Domains now live in `Domains`, one contiguous bit set of as many words per variable as the network needs, and `Domain` is a borrowed view of one variable's block. The width is a property of the network rather than of the type, so nothing about a word size refuses anything. The measured reason this is affordable rather than merely possible: over 77 real schemas the induced width is at most 2, a record-shaped schema is a star of induced width 1, and bucket elimination at width 1 costs `O((r + n) · d²)`, so a 200-line file against itself is a few milliseconds. The copy-on-branch the representation was chosen to protect is one `memcpy` and stays within noise of the single word it replaced, against an enforcement five orders of magnitude larger.
- **`⊥` is slot zero of every domain** (`panproto-mig`): it used to be the *last* slot, which is what tied its identity to the capacity and is why the capacity could not be removed without moving it. Bit zero is now `⊥` in every block, so "this vertex may still be dropped" is the low bit and the always-feasible all-`⊥` assignment is visible in the representation. The search's tie-break, "the lexicographically smallest assignment among the argmins", used to fall out of the numeric encoding and no longer can, so it is stated explicitly: `ValId::order_key` rotates the numbering, `Ord` is written by hand over it, and `DomainIter` walks the reals ascending and then `⊥`. `ValId::MAX_REAL_VALUES` and `Domain::CAPACITY` are gone, and `ValId::index()` now reads the real target index, so `⊥` looks up as absent rather than as the last target.
- **What refuses a search is a measured cost with a number in it** (`panproto-mig`): `CfnError::DomainTooLarge` is replaced by `CfnError::OverMemoryBudget`, which names the cost table entries the network would allocate, what they come to in bytes, and the budget they were checked against. `CfnBuilder::with_mem_bytes` and a new `mem_bytes` argument to `build_cfn` carry the caller's figure, and `SpanSearch::with_budget` now governs building the network as well as solving it, so a caller with the memory can search a pair that the default refuses. The iso path gets the same treatment: its three dense frames are sized by the two vertex counts alone and were allocated with no budget consulted, which at a few thousand target vertices is hundreds of megabytes; they are now checked before allocation and refused with `IsoError::OverMemoryBudget`. The default budget binds at roughly 2,900 same-kind vertices, some 46 times the old ceiling, and below it the dispatcher routes on measured cost rather than refusing at all. The dispatcher's own decomposition carries the caller's figure through to the per-component networks it builds, so raising the budget cannot cost a network the decomposition it qualifies for.
...
- **`find_morphisms` returns the optimal morphisms, not the whole hom-set** (`panproto-mig`, `panproto-c`, `panproto-py`): it used to enumerate every morphism, score each, sort, and truncate. It now returns the morphisms *attaining* the optimum. Every element therefore carries the same quality, which is the maximum over all total morphisms, so a caller reading element zero gets what it always got; a caller walking the list for a suboptimal alternative will not find one. Empty still means no total morphism exists.
- **`find_morphisms` returns a `MorphismList`, and the result cap bounds every request** (`panproto-mig`): `DEFAULT_OPTIMA_CAP`, which is 1024, applied only to `max_results = 0`, so the one request that named no figure was the only bounded one and any explicit figure was honoured verbatim. It now bounds every request, and `MorphismList::truncated` reports when it bound, which is the only way to tell a list the cap cut from a list the pair exhausted. The cap has to exist: the enumeration materialises one `FoundMorphism` per optimum and the count of optima is a property of the pair rather than of its size, so two eight-vertex schemas with no edges and no shared name characters tie the whole hom-set at the optimum, which is 16,777,216 morphisms, 4.6 GB and about three minutes uncapped against 4.6 MB and 11 ms capped. `find_morphisms_budgeted` and `find_best_morphism_budgeted` take the budget as an argument rather than spending `SearchBudget::default`, which is what a host bounding the search needs and what `SpanSearch::with_budget` already was for the span search.
- **A budget-stopped total-morphism search reports `SpanError::Stopped`** (`panproto-mig`, and every binding): branch and bound reaches complete assignments as it dives, so a budget spent before the first leaf leaves it with no incumbent, and the empty list that reported was a claim the search never established. The empty answer is now a statement about the pair and a refusal a statement about the budget, so shrinking a budget turns answers into refusals and never into wrong answers.
- **The span wire carries the apex digest and the functoriality flag** (`panproto-c`, `panproto-haskell`, `panproto-swift`): `SchemaSpanWire` carried nine of the certificate's ten fields' worth of measurements and left out the two a host needs to *identify* a span. There is no schema-digest entry point on the C ABI and the CBOR a host holds is not the digest's pre-image, so a Haskell or Swift host could not dedupe or cache a span it had just been handed, where Python and WASM could. Both new fields read with `serde(default)`, so a host encoding the other nine keys for `pp_hom_span_to_overlap` is still decoded rather than answered with a serialization error.
- **`discover_overlap` takes a protocol, returns a `Result`, and answers with the maximum common sub-schema** (`panproto-mig`): it ran the total-morphism search twice, once per direction, and kept whichever embedded more, so a pair where neither schema embeds wholly in the other came back as `SchemaOverlap::default()`. That is the ordinary case on the measured corpus, and it meant `schema_pushout` merged two schemas as though they shared nothing. It is now a single span search on the `iso` path, so the overlap is the largest common induced sub-schema. Both halves of the signature move: a `&Protocol` argument appears, because the apex is a schema and a schema is well formed only against a protocol, and the result is wrapped in `Result` so a search that could not be posed is reported rather than laundered into an empty overlap.
- **`DomainConstraints::scoring_weights` takes a `CostWeights`** (`panproto-mig`, and every binding): the field was a bare `[f64; 4]` multiplied into the score, so `[1.0; 4]` produced values up to 4.0 and an all-zero or negative vector was accepted silently. `CostWeights::new` normalises by the sum, rejects negative, non-finite and all-zero input with `CostWeightsError`, and carries a fifth `anchor` component the array had no slot for. Only the ratios of a weight vector now survive, and the score is confined to `[0, 1]`.
- **`merge::integrate_schemas` takes a protocol, and can fail before the pushout** (`panproto-vcs`): the third parameter is what `discover_overlap` now needs, and a failed overlap discovery surfaces as `VcsError::NotImplemented` naming the reason rather than silently producing a square built on an empty overlap.
- **`schema integrate` resolves the left schema's protocol on every path** (`panproto-cli`): it resolved one only under `--auto-overlap`, so a schema naming a protocol the CLI does not carry was integrated against an empty overlap without anything being consulted. It now exits non-zero naming the protocol and what is supported, on both paths.
- **The `HomBackend` typeclass gains `findSpan` and `spanToOverlap`, and a `ProtocolBackend` superclass** (`panproto-haskell`): neither method carries a default, so an out-of-tree instance must implement both and supply a `ProtocolBackend` instance the class did not previously require.
- **`WasmGlueModule` gains a required `auto_generate_span` member** (`@panproto/core`): `loadWasm` accepts a caller-supplied glue object, so a host passing a test double, a pre-instantiated module or a vendored build must add the member; without it `loadWasm` reads `undefined` at run time.
- **`SchemaDiff` declares what the extension has** (`panproto-py`): the stub declared `added_vertices` and `removed_vertices`, which always raised `AttributeError`, and omitted `classify`, which works. The two phantom attributes are gone and `classify(protocol, /) -> CompatReport` is declared.
- **Anchor resolution is aggregate-then-select** (`panproto-mig`): `align::resolve_anchors`, a per-source argmax over raw confidences, is replaced by `align::evidence::aggregate`, which reduces the whole anchor pool to one score per `(source, target)` pair, and `EvidenceTable::select`, which chooses off the search path. Aggregation applies a provenance ceiling, then a priority band, then a `max` within each of six evidence families, then a fixed-arity mean across them. The fixed arity is what makes the score monotone in the pool, and the monotonicity of the search optimum in how much evidence a caller supplies rests on it. `Anchor` gains a `provenance` field, stamped by the emitting branch rather than derived from the strategy tag, because one strategy can emit from two branches reading two different inputs.
- **Evidence reaches the search as a cost, never as a domain restriction** (`panproto-mig`): the solver chooses globally, subject to the hard constraints and the objective, rather than being handed a selection made for it in advance. That is what removes the failure where one high-confidence claim blocked a pair of moderate claims whose total was greater.
- **`schema auto-migrate` reports a span, and says how much of the source it covers** (`panproto-cli`): it asked for a total morphism and refused when none existed, which on real schema pairs is the ordinary answer rather than an unusual one. It now runs the span search, which never refuses, and the human path prints the apex size, the fraction of the source it covers, and the interval the search proved the quality lies in. Three flags form a strictness ladder over that one search: `--total` accepts only a span covering every source vertex, the default accepts any span covering at least one, and `--span` accepts an empty apex as the answer that the two schemas share nothing. `--monic` is orthogonal to all three, and `--json` now writes the span's right leg, a migration out of the apex, whose declared domain is the apex digest rather than the source schema. The command also resolves the source schema's protocol, because the apex is itself a schema and a schema is well formed only against a protocol, so a schema naming a protocol the CLI does not carry is now refused where it used to be searched.
- **A diff-derived migration reads a span rather than insisting on a total morphism** (`panproto-vcs`): `derive_migration` fell back to `find_best_morphism` when the diff showed both removals and additions, so rename detection only helped when the pair happened to admit a total morphism, which a change that also dropped a field never does. It now takes the right leg of the optimal span, which covers as much of the old schema as it can. Detected renames pin the search rather than steering it, since they are correspondences the crate computed rather than guessed. The adoption test is unchanged: the span-derived migration is used only when it maps more vertices than the diff-derived one and the spliced result type-checks as a theory morphism. The apex is validated against a protocol naming exactly the kinds the old schema uses, which makes the validation a statement about the induction rather than about how well a guessed protocol describes the schema in hand. `Repository::stage_schema` carried a second recovery pass over the same pair, adopting `find_best_morphism` when the diff-derived map covered under half the old schema; since that entry point answers only when a total morphism exists, it was silent on exactly the partial pairs rename recovery is for, and it is removed in favour of the one in `derive_migration`.

- **`auto_symmetric` refuses only when two schemas share no kind** (`panproto-lens`): it discovered the middle schema by matching vertex names, so two schemas that shared no name were reported as sharing nothing whatever their structure. The span search minimises a cost function in which pairing two kind-compatible vertices costs less than dropping both, so it answers with the sub-schema they do share, and the refusal is now reachable only when the optimal apex is empty. That takes two schemas whose kinds are disjoint, since an empty apex is otherwise never the cheapest assignment.

### Removed

- **`RecursionPoint::mu_id`** (`panproto-schema`, and the Python, Haskell, Swift and TypeScript surfaces): the marker vertex is the key the point is filed under in `Schema::recursion_points`, and naming it twice made the two copies independently settable, which deserialisation could not rule out. `SchemaDiff::added_recursion_points` and `removed_recursion_points` carry `(Name, RecursionPoint)` pairs instead, so a report still names the marker.
- **`SearchOptions::preferred`, `SearchOptions::max_nodes`, `SearchOptions::relax_edge_name_pruning` and `DomainConstraints::name_similarity_threshold`** (`panproto-mig`, and every binding): preferred mappings are gone because their job, soft evidence, is now a unary cost, which is strictly stronger: a preference could only change which optimum was found first, while a cost changes which assignment is optimal. The edge-name domain pruner is gone because it was a soft heuristic used as a hard filter, and edge-name agreement already enters the objective. The name-similarity threshold is gone because it cut a soft signal at a hard edge over full path-like identifiers rather than the local names a reader would compare, so it removed correct answers whenever two schemas agreed on a field and disagreed on the prefix leading to it. The node budget moved to `SearchBudget`, where exhausting it is reported through `SolveOutcome::limit_hit` rather than silently absorbed.
- **`SearchOptions::initial` is now `SearchOptions::hard_pins`** (`panproto-mig`, and every binding): renamed to say what it does. It is a hard restriction the search may not reconsider, not a starting point it may move away from. The C ABI wire field is `hard_pins`, and the Haskell, Swift and Python surfaces follow.
- **The soft-anchor retry and its node budget** (`panproto-lens`): `soften_seed_anchors` demoted strategy anchors from pins to preferences under `SOFT_ANCHOR_NODE_BUDGET`. With preferences gone there is no ordering left to express, so the retry now releases the strategy pins outright and keeps only the anchors the caller supplied. Releasing the pins is the part that mattered: it is the domain collapse, not the ordering, that turns a jointly infeasible anchor set into "no morphism found".
- **`AutoSpec::max_search_depth` is now `AutoSpec::max_results`** (`panproto-lens-dsl`): the field has always set `SearchOptions::max_results`, and the search is now an exact optimiser, so what it caps is the number of morphisms attaining the optimum rather than how far the search looks. The Nickel contract carries the new key under the same optionality.

### Documentation

- **`peak_cells` is described as what it counts** (`panproto-mig`): the elimination module claimed that because the counter reports the largest allocation the sweep made, "the claim is measured rather than asserted in prose". It is not: the counter is incremented from the length of the message a bucket returns, which is a quantity a join-then-minimise sweep would report identically, so no test built on it can tell the two shapes apart. Measuring it would need an allocation counter, which needs a global allocator, which the workspace's `unsafe_code = "deny"` rules out. The prose now says so, the test that named the claim is renamed to say what it checks, and two assertions comparing the same arities raised one power higher are removed: given the equality above them they are arithmetic identities and could never have failed.
- **The concatenation guard tests the prefix it names** (`panproto-schema`): `concatenation_is_unambiguous` compared a two-vertex schema against a one-vertex schema, so the differing element count already separated them and the string length prefix was never exercised; the test passed with every string prefix removed. It now also compares a pair with equal element counts whose runs concatenate alike, one vertex named `a` of kind `aab` against one named `aa` of kind `b`, which is the collision the rule exists to prevent. The module documentation's own example, one vertex named `ab` against vertices `a` and `b`, is not realizable for the same reason the old assertion was vacuous, and is replaced with the realizable pair.
- **The identity test asserts something that can fail** (`panproto-mig`): the optimality assertion compared the single element of a singleton hom-set against the maximum over that same set, which is `x >= x` whatever the objective reports; replacing the reported quality with a constant left it green. Four distinct kinds admit exactly one kind-respecting assignment, so the singleton is now asserted directly, which is the premise every other assertion in the test rests on, and the quality is pinned against the perfect-match ceiling computed from the weight vector rather than compared with itself.

- **The aggregation documents the formula it computes** (`panproto-mig`): the module stated `effective = lo(tag) + (1/14) * min(ceiling, confidence)` while the code computes `(13 - rank + capped) / 14`. The two are not equal in `f64`, and the documented form is not merely imprecise: at one rank and full confidence it returns a value strictly greater than that band's own top, a one-ulp escape into the band above that breaks the priority ordering at exactly the boundary the ordering is stated over. Anything reimplementing this in another language must divide once rather than multiply by a reciprocal and add.
- **The priority bands are closed intervals that meet, not half-open ones that are disjoint** (`panproto-mig`): both endpoints are attainable, so all 13 adjacent boundaries coincide bit for bit. Priority dominance survives as `≥` and the aggregation is unaffected, but the `max` within a family selects a tie rather than the higher-priority member at a shared endpoint, and `adjust_anchors_by_required_sets` clamps to exactly those two endpoints.
- **`Cardinality::Hybrid`'s `card` is an offset, not a count** (`panproto-mig`): it admits `card + 1` mappings per endpoint, matching AML's own selector, whose hybrid branch relaxes the strict bound by exactly one.
- **The tie-break among equally good assignments is read in decode order** (`panproto-mig`): the docs named the elimination order, and decoding walks it backwards, so the stated rule mispredicted the winner whenever the two disagreed. `SpanCertificate::tie_break_order` now reports the sequence actually used.
- **An empty overlap means no common *induced* sub-schema** (`panproto-mig`): inducing carries every arc between the chosen vertices, so a single self-loop on the target side makes an otherwise shared vertex unshareable. An empty overlap is not evidence that two schemas share nothing vertex by vertex.
- **`SchemaSpan::quality` includes the anchor term** (`panproto-mig`): the field documented the term as excluded, on the reading that evidence steers the search without inflating the number it reports. It does not: `Cfn::quality_of` reads the whole packed quality cost, and the weighted anchor cost is folded into that cost when the network is built, which `solve::build` already stated. The shipped anchor weight is zero, so the two readings coincide and no number was ever wrong; the doc misled anyone raising the weight, which is exactly the reader who needs it right.
- **A cross-module symbol link in the structural layer is a code span** (`bindings/swift`): `SchemaSpan.asTotalMorphism` linked `SchemaHandle/findBestMorphism(to:options:)` as a symbol. That symbol lives in the `Panproto` module, which `PanprotoStructural`'s documentation build does not see, so `swift package generate-documentation --warnings-as-errors` failed to resolve it and the documentation gate was red.
- **The generated CLI and C references match their implementations** (`book/`, `panproto-c`, `bindings/swift`): the CLI reference now carries the current strategy names, and both copies of the generated C header describe compiled lens-document handles and their ordered-stage instantiation. The book generator and header-drift gate now reproduce the committed files without a diff.
- **The book covers spans** (`book/`): a how-to page with a compiled example, covering what the apex is, what the three acceptance flags select, where a known correspondence goes as against an inferred one, how to reach the pushout from a span, and what each field of the certificate establishes. The CLI reference is regenerated from the new `schema auto-migrate` surface.
- **The stringency ladder is asserted on real lexicons, and the assertion runs by default** (`panproto-core`, `panproto-lens`): `stringency_monotonicity` was the only `#[ignore]` in the repository, and it asserted existence alone, that `auto_generate` returns `Ok` at two tiers on one lexicon pair. It now asserts, on that pair and on `app.bsky.feed.post` against `dev.panproto.translate.verifyCoercionLaws`, that the search proves its answer optimal at every tier, that the apex is non-empty and both legs functorial at every tier, that the optimal quality cost is monotone across the ladder compared on the integer cost rather than the `f64`, that the apex never shrinks, and that the whole test stays inside 100 ms in release, which is what lets the `#[ignore]` go. The second pair cost 24 seconds and 48 million search nodes under enumerate-then-sort and now costs single-digit milliseconds per tier.

  Two findings came out of writing it. First, the anchor weight ships at zero, so under the shipped weights the four tiers return the *same* span and a monotonicity assertion could not fail; the monotonicity test therefore weights the anchor term into the objective, where the optimum genuinely improves twice across the ladder on both pairs, and a separate test asserts the tier-invariance the shipped weights produce. Second, `Stringency` documents its tiers as a superset ladder and two strategies break it: `WlRefinement` takes an iteration count rather than a threshold from the tier, and a third round of colour refinement can separate a pair the second round matched; `Neighborhood` propagates from a one-to-one selection over the pool assembled so far, so a larger pool can give a source vertex a different seed and withdraw everything that propagated from the old one. Neither is a defect in the strategy, and neither can be made monotone without changing what it computes. The test asserts containment for every other strategy and asserts that those two are the only exceptions.

## [0.70.1] - 2026-08-12

### Added

- **The Swift release pin is under the version-consistency gate** (`.github/scripts/check_version_consistency.py`): `bindings/swift/Package.swift` names the published XCFramework by URL and checksum, and those two constants are what every SwiftPM consumer resolves against. Nothing checked them. They are also the one version field in the workspace that a machine writes after the tag rather than a bump before it, so `publish-swift.yml` failing left `main` pinned to a superseded XCFramework with CI green indefinitely, which is the shape of rot the script exists to catch. The check now asserts that the pin is well formed (the URL names this repository's releases and the XCFramework asset, the checksum is a SHA-256, and the two constants are empty only together), that it never names a release that has not been published, and that no release was published between the pinned one and the workspace version. That last rule is judged against the git tag list rather than against changelog headings, because a heading is written at bump time and so records intent: bumping twice before releasing, or abandoning a tag and superseding it with a patch bump, both leave a heading behind for a release that never shipped, and reading headings as releases would reject those legal states and block every pull request until someone released. A tag is what triggers the release, so it records fact. An empty pin is accepted only below the release in which the Swift package shipped, since at or above it an empty pin is a package that resolves to a system library no consumer has.

- **The check tests itself, and the test runs in CI** (`.github/scripts/check_version_consistency.py`): `--self-test` drives the Swift pin and documentation rules against a table of release states, covering the lag cases above, numeric version ordering (`0.9.0` precedes `0.10.0`, which string comparison gets backwards), every malformed-pin case, and the documentation patterns. It reads nothing from disk, so it runs as a flag on the script rather than as a harness with a dependency to install, and the `Version Consistency` job runs it alongside the check itself. That job now also fetches tags, without which the check can still see a forward or malformed pin but not a skipped release; it names the record it had in its summary line rather than degrading quietly.

### Fixed

- **The Swift install instructions no longer name a release** (`bindings/swift/README.md`, `book/src/how-to/install/swift.md`, `bindings/swift/bootstrap/fetch-bindist.sh`): four hard-coded tags had rotted, three of them a minor behind the workspace and all four disagreeing with each other, so a reader following any of them fetched an engine older than the checkout. `fetch-bindist.sh` already defaulted its version to the workspace `Cargo.toml`, but its feature set was the second positional argument, which meant naming a variant forced you to name a version too. Arguments are now recognized by shape rather than by position, so `fetch-bindist.sh full` and `fetch-bindist.sh --xcframework` select a variant while leaving the version to the checkout, and the documentation uses those forms. A tag that is not shaped `vX.Y.Z` is now rejected with a message instead of becoming a 404 on a URL built from it; `--tag` passes one through for a `PANPROTO_REPO` fork whose tags follow another convention. The version-consistency check covers these files, so a tag written back into them fails CI.

- **The release workflow and the gate read the Swift pin the same way** (`.github/workflows/publish-swift.yml`): three patterns matched the same two constants with three different tolerances. The mirror guard's `sed` required the declaration to start at column zero and end at the closing quote, so a trailing comment or a stray space would have failed the release with an error naming a race that had not happened, while the rewrite step and the check were both satisfied; an indented declaration would have gone the other way, leaving CI red over a pin the workflow was rewriting correctly. All three now accept leading whitespace and trailing content, and the rewrite is anchored to the start of a line and preserves the indentation it found. A rewrite that matches nothing is now a hard failure rather than a silent no-op, so renaming a constant cannot publish a release still pinned to the previous XCFramework.

- **The pin cannot be moved backwards** (`.github/workflows/publish-swift.yml`): the `pin` job checks out `main` rather than the tag it is publishing, so re-running the workflow for an older tag, which is how an operator recovers a release whose mirror step failed, rewrote `main`'s pin backwards and pushed it, stranding every SwiftPM consumer on a superseded XCFramework. The job now refuses to move the pin to an older release and says so.
...
- **A project whose files agree on a protocol keeps that protocol** (`panproto-vcs`): a multi-document project was assembled under an internal coproduct protocol named `project`, which carries no edge rules, object kinds or constraint sorts. Equation diagnostics are selected by protocol name, so a project whose every file was ATProto reported `no protocol theory registered for 'project'; schema equations were not checked` and was still marked valid: the protocol's theory was never consulted. An assembled project now takes the protocol its files agree on, so a homogeneous project is checked against that protocol's theory and a violation blocks the commit instead of passing unexamined. A project that genuinely mixes protocols still has no single theory to check against, and now says so, naming the protocols it mixes rather than the internal coproduct; a homogeneous project in a protocol with no registered theory names that protocol. `schema show` reports the project's own protocol as well.

- **`schema compat` and `schema diff` accept project directories** (`panproto-cli`): both commands deserialized each operand directly as panproto's own serialized schema, so the only input they accepted was a file already in panproto's internal JSON. Comparing two versions of a lexicon project, the ordinary reason to run a compatibility check, was impossible. Each operand now goes through a shared loader that also accepts a manifest-backed project directory (parsed as one bundle, so references resolve across documents and the comparison sees the same assembled schema `add` stages), a directory with no manifest in the protocol named by `--protocol`, a single schema document, and a source tree. The exit codes are unchanged: `0` no breaking changes, `1` breaking changes, `2` a usage or load error. A manifest is authoritative about its own protocol, so a `--protocol` that disagrees with it is a load error rather than a silent override.
...
- **The hard-coded protocol specs are gone from the TypeScript SDK** (`@panproto/core`): `ATPROTO_SPEC`, `SQL_SPEC`, `PROTOBUF_SPEC`, `GRAPHQL_SPEC`, `JSON_SCHEMA_SPEC` and `BUILTIN_PROTOCOLS` are no longer exported. They duplicated definitions that live in `panproto-protocols`, and duplicating them is what let ATProto drift. Read a built-in protocol with `getBuiltinProtocol(name, wasm)` or `panproto.protocol(name).spec`, both of which read the registry.

## [0.70.0] - 2026-08-11

### Added

- **panproto is reachable from Swift** (`bindings/swift`): a SwiftPM package linking `libpanproto_c`, binding all 120 C ABI entry points, which is the same surface the Haskell binding consumes, so the two are at parity with each other and with the ABI. The Python SDK is a superset of both: `panproto-py` is a PyO3 extension linking `panproto-core` directly, so it reaches 52 members with no `pp_*` entry point behind them (schema-document and IDL parsing for 106 protocols, version-control porcelain beyond the 13 operations the ABI carries, theory-DSL loading, hint-steered lens generation, and runtime grammar override). That delta is architectural and affects Haskell identically; the Swift SDK reference names each group. Swift reaches seven surfaces Python does not: instance queries, the graph fiber calculus, schema enrichment, the dataset and staleness layer, symmetric lenses, evaluation in a model, and expression typechecking. Six products split along two lines: whether a tier needs the engine (`PanprotoStructural` does not; `Panproto`, `PanprotoVcs`, `PanprotoParse`, `PanprotoProject`, and `PanprotoGit` do), and whether its entry points exist in the default cdylib (parse, project, and git do not). The gated products stay in the package graph either way, with their sources compiled out, so resolution never depends on how the library was built.

  Engine work runs on `PanprotoEngine`, a global actor pinned to one thread. The slab is process-global and mutex-guarded, so that is not what needs pinning; the last-error slot is thread-local, and every error message the binding reports depends on the drain landing on the thread that failed. A serial queue would give mutual exclusion but not thread identity, so the invariant would hold only until some future call suspended between the failure and the drain. Handles own one slab entry each and release through the executor's queue, since a deinitializer cannot suspend. `PanprotoError` has twelve cases and every method is declared `throws(PanprotoError)`, with the domain supplied by the call site (so it is exact rather than inferred) and a structured `Fault` recovered from the envelope where the engine's message is specific enough to recognize.

  CBOR is written against `ciborium`'s rendering of serde's data model rather than as a general-purpose codec, because that is the only shape the ABI speaks. Conformance is checked by decoding a captured engine payload, re-encoding it, handing it back to the engine, and reading it out again: bytes the engine rejects are the failure the wire layer exists to prevent. It compares decoded values rather than bytes, because most schema fields are `HashMap`s and `ciborium` writes them in iteration order, so the engine does not emit one schema as the same bytes twice.

  Three CI gates keep it honest: the regenerated header must match the copy the package compiles against, every ABI entry point must have a shim and every shim a caller and a test, and `swift-format` runs with documentation required on every public declaration.

- **The Swift value layer carries the algebra offline** (`bindings/swift`): `PanprotoStructural` renders expressions back to surface syntax, composes migration specifications with the engine's drop-on-miss chasing (plus the field-level combinators built on it), treats a protolens chain as a monoid that folds to an optic kind, composes schema morphisms as partial maps, and diffs two schemas. None of it starts an engine, which is what the product is for.

### Fixed

- **A more permissive lens search no longer fails where a stricter one succeeded** (`panproto-mig`, `panproto-lens`): `Stringency` documents that higher tiers form a superset of lower ones, and they did not. Alignment strategies wrote their anchors into `SearchOptions::initial`, which collapses a vertex's CSP domain to exactly the pinned target. A tier that runs more strategies contributes more anchors, and anchors that are individually plausible can be jointly infeasible: on two ATProto lexicons, two of them together require a source edge to map to a target edge that does not exist. Anchors are scored one at a time, so nothing ever evaluates the conjunction, and `Exploratory` reported no morphism on a pair `Lenient` aligned. `SearchOptions::preferred` now carries strategy output as an ordering over a domain rather than a collapse of it, so a wrong guess costs the order the solver tries targets in and never a solution; `initial` keeps its hard semantics for anchors a caller supplies, which are known rather than inferred. Refs #258.
- **An excluded source no longer vetoes the whole search** (`panproto-mig`): `BacktrackState::new_constrained` removed an excluded vertex from the domain map but built the assignment from `initial` wholesale, so the vertex stayed assigned, kept its target marked used, and kept its edges owing an image in `build_morphism_weighted`, which then returned `None`. A source the caller asked to drop could make the search fail.
- **A pinned anchor no longer bypasses the kind check** (`panproto-mig`): every other path into a CSP domain admits only kind-compatible targets and `build_morphism_weighted` relies on that, but the pin path collapsed the domain with no check, so a pin could map an integer vertex onto a string one. An incompatible pin now yields an empty domain and fails the search.
- **The search node budget is enforced on both search functions** (`panproto-mig`): `max_nodes` was read only by `backtrack`, and `find_morphisms_constrained` calls `backtrack_weighted`, which never checked it. Since `run_search` takes the constrained path whenever span exclusions exist, which is the normal case at `Lenient` and above, the budget was dead code exactly where it was needed.
- **Detached HEAD crosses the C ABI as hex** (`panproto-c`): the vcs module states that object ids cross as their lowercase-hex rendering, but the status and op-result records held `vcs::HeadState` directly, whose `Detached` variant carries an `ObjectId`, a newtype over `[u8; 32]` that serde encodes as a 32-element array. A host therefore had to decode a digest one way in `head_commit` and another in `head_ref`, and the Haskell decoder, which reads `Detached` as a text string, failed outright on a detached HEAD.

### Documentation

- **The C ABI contract matches the ABI** (`panproto-c`): `CONTRACT.md` documented 96 default entry points where the header declares 103. Five `gat` entry points, `pp_mig_serialize_compiled`, and `pp_lens_compile_document_with_refs` were missing entirely, and the handle taxonomy did not list `Model`, which `pp_gat_free_model` allocates. The committed header was also a revision behind the crate.
- **The book covers Swift** (`book/`): a Swift SDK reference, an install page, and a define-a-schema how-to, plus Swift and Haskell added to the install, define-schema, and reference index tables, which listed only Rust, TypeScript, and Python.
## [0.69.1] - 2026-08-11
