...
- **Emit coverage: 30 more long-tail protocols promoted to `Verified`** (`panproto-parse`): a third sweep over the previously-unsampled tail promoted apex, arduino, blade, caddy, chatito, cooklang, corn, cpon, cylc, dtd, earthfile, elixir, enforce, faust, gdscript, gstlaunch, hack, hlsl, lilypond, linkerscript, luadoc, mojo, move, nqc, puppet, qml, query, re2c, solidity, and verilog under the stronger oracle. `VERIFIED_EMIT_PROTOCOLS` is now 148 of 261.
- **Emit coverage: 40 more long-tail protocols promoted to `Verified`** (`panproto-parse`): an expanded fixed-point sweep over the untested tail promoted actionscript, agda, al, asciidoc, bass, beancount, desktop, devicetree, elisp, firrtl, func, gleam, groovy, heex, idris, ispc, janet, jq, lua, luau, matlab, netlinx, objc, pony, postscript, powershell, qmldir, r, racket, rego, rescript, scala, sparql, squirrel, tablegen, tcl, templ, tmux, twig, and v — each verified under the stronger oracle (byte fixed point + kind-multiset + edge-multiset preservation). `VERIFIED_EMIT_PROTOCOLS` is now 113.
- **Emit coverage: 30 long-tail protocols promoted to `Verified`** (`panproto-parse`): a 126-protocol fixed-point sweep promoted asm, bicep, capnp, cmake, dockerfile, ebnf, editorconfig, fidl, forth, git_config, git_rebase, gitcommit, glsl, graphql, just, llvm, nginx, nickel, org, pkl, requirements, smithy, starlark, supercollider, textproto, thrift, ungrammar, vim, wast, and wat, bringing `VERIFIED_EMIT_PROTOCOLS` to 68. New test `emit_long_tail.rs` uses a **stronger oracle** than the byte fixed point alone: it additionally asserts the parse round-trip preserves the schema's vertex-kind and edge-shape multisets, rejecting *degenerate* fixed points (an emitter that drops content to `""` satisfies the byte law trivially but loses structure). The sweep also established that all 261 vendored grammars carry `grammar.json` (emit is structurally possible) and only `less` is ABI-incompatible with tree-sitter 0.26.
- **Emit coverage: 19 protocols promoted to `Verified`** (`panproto-parse`): fixed-point tests (`emit(parse(emit(s))) == emit(s)`) now cover ada, css, fortran, fsharp, hare, haskell, hcl, html, json, jsonnet, latex, markdown, nim, nix, ocaml, svelte, toml, yaml, and zig, bringing `VERIFIED_EMIT_PROTOCOLS` from 16 to 35. Protocols with a remaining emit defect (scss/tsx/vue/astro/elm/d/odin/purescript/lean/kdl/ron/typst spacing or fixed-point gaps) carry `#[ignore]` tests documenting the observed symptom for follow-up.
...
- **Same-kind alias dispatch corrupted operators** (`panproto-parse`): a grammar may alias two different source rules to the same node kind — Ruby aliases `command_binary` (keyword operators `and`/`or`) to kind `binary`, while the arithmetic `binary` rule (`+`, `-`, `*`, …) also surfaces as `binary` — so the emitter's direct-name and subtype passes matched the first alias by its value and walked the wrong source, turning `x + 1` into `x or 1`. `pick_choice_with_cursor` now consults the child vertex's recorded operator witness (`chose-alt-fingerprint`): an aliased source carrying its own keyword literals is accepted only when one of them appears in that witness, so `x + 1` reaches the arithmetic rule and `x and y` still reaches the command rule. Applied in the direct-name, subtype, and yield-set passes. (Derived from parse witnesses, not language-specific.)
- **CHOICE dispatch stole an edge from a later mandatory member** (`panproto-parse`): the emitter's subtype closure `subtypes[K]` is a deep-reachability relation, so an optional alternative that wraps the target kind in its own node was wrongly selected for a child that belonged to a later member — D's `declarator = SEQ[identifier, CHOICE[template_parameters | BLANK], "=", _initializer]` picked `template_parameters` for the initializer's `int_literal` (a template value argument can be an `int_literal`), emitting `int x 1 =;` and dropping the assignment. `pick_choice_with_cursor` now consults the `chose-alt-child-kinds` parse witness: a self-anchored concrete named rule (one carrying its own literal tokens, so it always materialises under its own name) that is absent from the recorded child kinds was not taken, and is skipped in both the subtype and yield-set passes. Hidden/supertype symbols and pure-symbol wrappers (which inline transparently, e.g. Julia's `macro_argument_list`) are exempt. Promotes d to `Verified`; this is the witness-driven dispatch fix the lossy fingerprint approach could not achieve. (Derived from parse witnesses, not language-specific.)
- **Alternation/character-class newline terminators emitted as a `_` placeholder and grew** (`panproto-parse`): a structural line-break terminal written as an alternation (CSV/TSV row end `\r|\r\n|\n`) or a newline-only character class (`properties`' `[\r\n]|\r\n`) was not recognised by the newline detector, so it fell through to the `_` placeholder; the `_` then re-parsed into a phantom field and accreted one per emit (`a,b,c` → `a,b,c _ 1,2,3 _` → …). `is_newline_like_pattern` now accepts a top-level alternation whose every branch is newline-only (`split_top_level_alternation`, ignoring escaped/`[...]`-internal `|`) and newline-only character classes, emitting a real line break. Promotes csv, tsv, and properties to `Verified`. (Derived from the grammar's regex, not language-specific.)
- **Values spaced from `=` grew on each emit** (`panproto-parse`): a content terminal whose grammar `PATTERN` can match a leading space (INI's `setting_value = PATTERN ".+"`, where `.` matches a space) captures that space, so when the emitter inserts a layout space *and* the captured text already begins with one, `key = value` re-emits as `key =  value` then `key =   value` — the same unbounded-growth class as the Python string and HTML text fixes, reached through ordinary `=`-separated config values. The grammar is now scanned for such terminals (`pattern_absorbs_leading_space`: a `.`-led pattern, or a negated class that does not exclude whitespace) and recorded in `Grammar::leading_space_terminals`; when one of those terminals captured leading whitespace, the emitter suppresses the redundant layout space (the captured whitespace is the separator) on every leaf path (`emit_vertex`, `emit_aliased_child`). A layout space is still emitted when the captured text carries no leading whitespace, so genuine separators (Org's `* Heading`) are preserved. Promotes ini and abc to `Verified`. (Derived from grammar facts, not language-specific.)
- **HTML element text grew on each emit** (`panproto-parse`): a `<p>Hi</p>` element re-emitted as `<p> Hi </p>` then `<p>  Hi  </p>` — the layout pass spaced the element `text` from the surrounding `>` / `</`, and those spaces folded into the re-parsed text node (the same unbounded-growth class as the Python string fix). A new cassette hook (`GrammarCassette::kind_is_tight_content`) lets the HTML family declare `text` / `raw_text` as raw inline content; such leaves now emit tight on both sides. Promotes vue to `Verified`; keeps html/svelte fixed points without inserted whitespace.
- **Elm declarations collapsed onto one line** (`panproto-parse`): Elm's layout scanner separates top-level declarations with the external `_virtual_end_decl` (not the conventional `_newline`), which the generic newline-classifier could not recognise, so the emitter produced no line break and `module … x = 1` collapsed and failed to re-parse. A new cassette hook (`GrammarCassette::external_is_newline`) lets a grammar declare such layout terminators; the Elm cassette maps `_virtual_end_decl` / `_virtual_end_section` to a newline. Promotes elm to `Verified`. (Keeps language-specific layout naming in the cassette, not the generic emitter.)
- **Statement separator flipped `\n`↔`;` on re-emit** (`panproto-parse`): a pure-separator `CHOICE` (all alternatives emit no named child, e.g. Odin's `_separator = CHOICE[_newline, ";"]`) was decided by the vertex's literal fingerprint, which a `;` *elsewhere* in the vertex could contaminate — so a declaration separator emitted as a newline on the first pass became `;` on the re-emit, breaking the fixed point. Such separator CHOICEs now prefer a newline alternative (a `Production::Pattern` newline or an external `_newline`-family token) when a separator is genuinely needed. Promotes odin to `Verified`.
- **Case-insensitive keywords emitted as `_` placeholders** (`panproto-parse`): grammars that spell keywords as case-insensitive patterns aliased to a canonical word (`ALIAS{ PATTERN "[pP][rR]…", value: "procedure" }`, as Ada, Fortran, and Nim do) emitted a `_` placeholder for the pattern and then collapsed on re-parse. The anonymous-alias emit path now emits the alias `value` when its content is a bare terminal PATTERN with no captured literal, so the keyword text is reproduced and re-parses to the same kind. Promotes ada, fortran, and nim to `Verified`.
- **Emit stack overflow on grammars with cyclic hidden rules** (`panproto-parse`): the CHOICE-dispatch acceptance predicate `accepts_first_edge` followed hidden/supertype `SYMBOL → rule` references with no cycle guard, so a mutually-recursive hidden-rule graph (e.g. Zig's declaration rules) recursed until the stack overflowed and aborted the process, even on trivial input. The expansion now carries a visited set (mirroring `yield_of_production`), bounding the walk. Zig now emits as a byte fixed point and is `Verified`.
- **Bracket pairs mis-detected when flanked by other tokens** (`panproto-parse`): the emitter's bracket-pair detector took the first and last STRING of a SEQ, so a rule like Stan's `sampling_statement` (`expr ~ f ( args ) ;`) paired `~` with `;` instead of `(` with `)` — emitting `y~normal ( 0,1 )` with the callee detached and the arguments loosely spaced. A canonical pass now pairs an actual `(`/`[`/`{` with its matching closer before the first/last fallback, so the callee hugs its argument list (`normal(0, 1)`). Empty bracket-pair literals (`()`, `[]`, `{}`) likewise hug their callee (`f()`, not `f ()`) while still spacing after a keyword (`return ()`).
- **Python function bodies lost their indentation** (`panproto-parse`): a multi-statement suite emitted with `return` at column 0 (structurally outside the function) because the `block` body vertex is reached directly — the hidden `_suite` wrapper carrying the opening `_indent` is not a vertex, so no indent scope opened. A rule whose body references an external indent-close token (`_dedent`) but no indent-open token is now recognized as an indented block, and the emitter synthesizes the opening indent when it walks the rule; the rule's own `_dedent` closes it. Function bodies emit indented and as a byte fixed point. Closes #160.
- **Python string literals accumulated interior whitespace** (`panproto-parse`): `'hello'` re-emitted as `' hello '`, then `'  hello  '`, growing two characters per round trip, because the `string_start`/`string_end` scanner delimiters were emitted as free-standing terminals and the layout pass spaced them from the content; the spaces then folded into the re-parsed `string_content`. Delimiter externals that structurally bracket their content (`SEQ[string_start, REPEAT(content), string_end]`) are now derived from the grammar and emitted with tight bracket roles, so string literals are a byte fixed point of stable length. Closes #182.
...
- **C++-scanner grammars failed to build** (`panproto-grammars`): grammars with a `scanner.cc` that `#include`s C++ standard headers (Mojo, Norg, Wolfram) failed to compile because the namespace wrapper pulled those headers inside the per-grammar namespace, breaking libc++ internals (`using ::nullptr_t;`). The wrapper now hoists the scanner's angle-bracket system includes to global scope so their include guards neutralize the in-namespace re-include. Symbol localization is skipped for these grammars (the namespace already isolates them, and their parser archive's external-scanner references are undefined at partial-link time). On macOS the active SDK's libc++ include directory is added defensively so toolchains that omit it from clang's default search still compile. All three grammars now parse.
