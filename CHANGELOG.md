...
- **`find_morphisms` returns the optimal morphisms, not the whole hom-set** (`panproto-mig`, `panproto-c`, `panproto-py`): it used to enumerate every morphism, score each, sort, and truncate. It now returns the morphisms *attaining* the optimum, capped by `max_results`. Every element therefore carries the same quality, which is the maximum over all total morphisms, so a caller reading element zero gets what it always got; a caller walking the list for a suboptimal alternative will not find one. Empty still means no total morphism exists.
