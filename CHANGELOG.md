...
- **Per-(platform, group) companion publish workflow** (`.github/workflows/python-wheels-companions.yml`): on a `v*` tag push, builds and publishes one wheel per (target-platform, companion) pair. Runs `tools/fetch-grammars.py` for groups whose grammars are fetched at CI time rather than vendored.
