...
Companion grammar pack for [panproto] shipping tree-sitter grammars for data-language source: JSON, TOML, XML, YAML, SQL, CSV, GraphQL, and Protobuf.
...
Pre-1.0 with arbitrary breaking changes between minor versions. The pack version tracks the workspace `panproto` version on every release; consumers should pin both to the same minor (e.g. `panproto>=0.45,<0.46` and `panproto-grammars-data>=0.45,<0.46`).
...
Source: [`crates/panproto-grammars-data`](https://github.com/panproto/panproto/tree/main/crates/panproto-grammars-data) on the Rust side, [`bindings/python-grammars-data`](https://github.com/panproto/panproto/tree/main/bindings/python-grammars-data) on the Python side.
