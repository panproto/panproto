...
Companion grammar pack for [panproto] shipping tree-sitter grammars for jvm-language source: Java, Kotlin, Scala, Groovy, and Clojure.
...
Pre-1.0 with arbitrary breaking changes between minor versions. The pack version tracks the workspace `panproto` version on every release; consumers should pin both to the same minor (e.g. `panproto>=0.45,<0.46` and `panproto-grammars-jvm>=0.45,<0.46`).
...
Source: [`crates/panproto-grammars-jvm`](https://github.com/panproto/panproto/tree/main/crates/panproto-grammars-jvm) on the Rust side, [`bindings/python-grammars-jvm`](https://github.com/panproto/panproto/tree/main/bindings/python-grammars-jvm) on the Python side.
