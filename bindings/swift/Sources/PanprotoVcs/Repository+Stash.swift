...
extension RepositoryHandle {
    ...
    /// object store, which is what a caller stages again by loading that
    /// schema and passing it to ``add(_:)``; the pop itself leaves the
    /// index as it found it. ``VcsStashPopResult/stack`` is read from
    /// the reflog, which keeps the entry the pop consumed, so it reports
    /// the pushes this repository has seen rather than what is still
    /// poppable.
    ///
    /// - Returns: the popped schema's object id alongside the reflog.
    /// - Throws: ``PanprotoError/vcs(_:)`` when `refs/stash` names
    ///   nothing, which is the state before the first ``pushStash()`` and
    ///   after the last pop.
    @PanprotoEngine
