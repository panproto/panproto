...
extension RepositoryHandle {
    ...
    /// repository with no commits has no branches yet, so the listing is
    /// empty.
    ///
    ...
    ///   is no commit to branch from, or when the name is already taken.
    @PanprotoEngine
    ...
    /// fast-forwards when HEAD is already an ancestor of the branch tip,
    /// which ``VcsMergeResult/fastForward`` reports, and otherwise
    /// records a merge commit attributed to `author`. A merge that
    /// conflicts leaves HEAD where it was and describes each conflict in
    /// ``VcsMergeResult/conflicts``, with no merge commit to name.
    ///
    /// - Parameters:
