...
Companion grammar pack for [panproto] shipping tree-sitter grammars for scripting-language source: Python, Ruby, Lua, Bash, Perl, R, Julia, Nushell, and Fish.
...
Pre-1.0 with arbitrary breaking changes between minor versions. The pack version tracks the workspace `panproto` version on every release; consumers should pin both to the same minor (e.g. `panproto>=0.45,<0.46` and `panproto-grammars-scripting>=0.45,<0.46`).
...
Source: [`crates/panproto-grammars-scripting`](https://github.com/panproto/panproto/tree/main/crates/panproto-grammars-scripting) on the Rust side, [`bindings/python-grammars-scripting`](https://github.com/panproto/panproto/tree/main/bindings/python-grammars-scripting) on the Python side.
