...
Companion grammar pack for [panproto] shipping tree-sitter grammars for devops-language source: Dockerfile, Terraform, HCL, Nix, Bash, YAML, TOML, Make, and CMake.
...
Pre-1.0 with arbitrary breaking changes between minor versions. The pack version tracks the workspace `panproto` version on every release; consumers should pin both to the same minor (e.g. `panproto>=0.45,<0.46` and `panproto-grammars-devops>=0.45,<0.46`).
...
Source: [`crates/panproto-grammars-devops`](https://github.com/panproto/panproto/tree/main/crates/panproto-grammars-devops) on the Rust side, [`bindings/python-grammars-devops`](https://github.com/panproto/panproto/tree/main/bindings/python-grammars-devops) on the Python side.
