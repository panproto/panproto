...
|---|---|
| `src/lib.rs` | Public API re-exports |
| `src/wtype.rs` | W-type instance: `compute_surviving_set()`, `anchor_nodes()`, `contract_ancestors()`, `resolve_edges()` |
| `src/fan.rs` | `Fan` struct, `reconstruct_fans()` |
...
## crates/panproto-protocols --- Built-in protocols

Concrete protocol implementations for supported schema languages.

| File | Description |
|---|---|
| `src/lib.rs` | Public API, protocol registry |
| `src/theories.rs` | Shared base theories (e.g., the "typed graph" sub-theory) |
| `src/atproto.rs` | AT Protocol (Lexicon): `build_atproto_protocol()` |
| `src/protobuf.rs` | Protocol Buffers: `build_protobuf_protocol()` |
| `src/graphql.rs` | GraphQL: `build_graphql_protocol()` |
| `src/json_schema.rs` | JSON Schema: `build_json_schema_protocol()` |
| `src/sql.rs` | SQL DDL: `build_sql_protocol()` |
...
| File | Description |
|---|---|
| `src/lib.rs` | Re-exports from `panproto-gat`, `panproto-schema`, `panproto-inst`, `panproto-mig`, `panproto-lens`, `panproto-check` |
