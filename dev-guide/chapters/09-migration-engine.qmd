...
panproto derives its validation rules from the protocol's algebraic theory rather than hardcoding them, making the same engine work across all 60+ supported protocols.
...
The existence checker is one of the most distinctive parts of panproto's migration engine. Rather than hardcoding a fixed set of validation rules, `check_existence` inspects the protocol's schema and instance theory sorts at runtime to determine which checks apply. The validation rules are derived from the structure of the protocol's theory, so adding a new protocol with novel theory composition requires no code changes to `panproto-mig`.^[Rather than hardcoding validation rules, panproto derives them from the protocol's theory structure --- checking constraints only when the theory declares a Constraint sort, checking hyper-edge signatures only when the theory declares a HyperEdge sort. This approach is inspired by the double-pushout rewriting framework [@ehrig2006], where validation conditions are derived from the structure of the rule rather than enumerated by hand.]
...
- **`Node` sort present in instance theory**: enables reachability risk detection for tree instances. When intermediate vertices are dropped, subtrees that were connected through those vertices may become disconnected from the root, producing orphaned data.
...
::: {.callout-tip}
If your migration has already passed existence checking, compilation will not fail unless the migration references vertices or edges absent from the target schema (which existence checking should have caught). Always run existence checking first---it produces much more informative error messages than compilation failures.
:::
...
Resolvers from both migrations are merged, with $m_2$'s resolvers taking precedence when keys overlap. This is correct because the second migration's resolvers encode decisions about the final target schema, which take precedence over the intermediate schema's structure.

Composition is associative: $(m_3 \circ m_2) \circ m_1 = m_3 \circ (m_2 \circ m_1)$. Combined with the identity migration as the unit, this makes migrations composable in the expected way.^[Formally, migrations form a category---the *migration category* over a fixed protocol.]
...
When the VCS merge algorithm composes migration paths through different branch histories, it must verify that the composed path is coherent -- that the composed maps commute with all operations.^[The composed migration path must produce the same result regardless of which order the individual steps are applied. In categorical terms, this is the naturality condition for natural transformations between the composed morphisms.]

The `check_natural_transformation` function from `panproto-gat` is used at two points in the merge pipeline:

1. **Three-way merge verification**: after computing the merged schema, the merge algorithm constructs compatible families of maps^[These are families of maps that are compatible with all operations in the theory --- meaning the maps commute with every operation. In the categorical literature, these are called natural transformations. See the tutorial's [Appendix A](https://panproto.dev/tutorial/appendices/A-formal-foundations.html).] from the "ours" and "theirs" migration morphisms to the merge result. These must commute with all operations for the merge to be semantically correct.
...
The merge result struct includes a `pullback_overlap` field that reports the shared structure^[The shared structure is the set of elements that both branches agree on --- elements that map to the same thing in the merge base. Categorically, this is a pullback.] of the two branch theories over the merge base theory. Non-empty overlap indicates shared structure that both branches modified, which requires careful reconciliation.
...
- **`panproto-gat`** provides the `Theory` type used for existence checking and the merge function used by `panproto-protocols`.^[This merge combines two theories along their shared structure, producing a combined theory with all declarations from both. Categorically, this is a colimit (pushout).]
...
- **Lifting** is O(|instance nodes|) per record, with hash-map lookups for remap and resolver operations. For tree instances, this is a single tree traversal. For table instances, this is a single pass over rows.
