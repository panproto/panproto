...
This chapter covers the object model, algorithms, and integration points for data versioning in `panproto-vcs`. For the user-facing workflow, see the [tutorial's data versioning chapter](https://panproto.dev/tutorial/chapters/19-data-versioning.html).
...
The `Protocol` variant stores a complete `panproto_protocols::Protocol` value. This is the same struct used throughout the codebase---no wrapper or subset. Serialization uses the existing `serde` implementation with MessagePack encoding.
...
## Hash functions: canonical forms for new types {#sec-data-hash-functions}

**File**: `crates/panproto-vcs/src/hash.rs`

Content addressing requires a canonical serialization. The existing `hash_object` function dispatches on the `Object` variant. Three new arms handle the new types:
...
Backward migration fails with `Error::MissingComplement` if the complement object cannot be found in the store. Lossless steps do not need complements---the inverted migration can reconstruct the original data without external input.
...
The function does not migrate data. It is a read-only diagnostic.
...
:::{.callout-note}
The `Index` is serialized as JSON to `.panproto/index.json`. The new fields use `serde(default)` on `staged_data` (defaults to empty vec) but `staged_protocol` is `Option<Protocol>` (defaults to `None`). This means old index files missing these fields will deserialize with empty data staging, but the `Index` struct itself is a breaking change because the `CommitObject` it produces now requires the new fields.
:::
