...
Loads the schema from JSON, resolves the protocol by name, and runs protocol-aware validation. In addition to structural validation, `schema validate` now also type-checks the protocol's schema theory equations via `gat_validate::validate_theory_equations`. Exits with code 1 on failure. Calls `panproto_core::schema::validate`.
...
This command is the standalone equivalent of the type-checking that `schema add` and `schema commit` perform automatically. It is useful for CI pipelines that want to validate schemas without a `.panproto/` repository.
