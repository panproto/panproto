...
1. **`compute_surviving_set()`** in `crates/panproto-inst/src/wtype.rs` --- Determines which nodes survive the migration. If your output is missing data, check whether the surviving set is too small.

2. **`anchor_nodes()`** in `crates/panproto-inst/src/wtype.rs` --- Assigns surviving nodes to their new positions in the target schema. If nodes appear at wrong vertices, the anchor computation is the place to look.

3. **`contract_ancestors()`** in `crates/panproto-inst/src/wtype.rs` --- Merges ancestor chains when intermediate vertices are removed. Panics here usually mean the surviving set is inconsistent with the vertex remap.

4. **`resolve_edges()`** in `crates/panproto-inst/src/wtype.rs` --- Rewires edges according to the compiled migration's edge remap. If edges point to wrong targets, inspect the resolver.

5. **`reconstruct_fans()`** in `crates/panproto-inst/src/fan.rs` --- Rebuilds fan structures (ordered child collections) after restrict. Missing or reordered children point to fan reconstruction issues.
...
1. **Check the surviving set.** Run with `RUST_LOG=panproto_inst=debug` and look at the `compute_surviving_set` output. Are all expected nodes present?
