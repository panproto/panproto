...
::: {.callout-warning}
The standalone `combinators.rs` module has been superseded by protolenses (@sec-protolens-engine). The combinator semantics described below are now implemented as elementary protolens constructors in `protolens::elementary` (e.g., `elementary::rename_sort`, `elementary::drop_sort`, `elementary::add_sort`). See @sec-elementary-protolens for the full list.
:::

```rust
// Conceptual combinator enum (now implemented via protolens::elementary)
pub enum Combinator {
