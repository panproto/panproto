...
The crate root re-exports `Lens`, `Complement`, `get`, `put`, `Protolens`, `ProtolensChain`, `ComplementConstructor`, `auto_generate`, and the composition functions for convenient access.
...
The `classify_transform` function maps each `TheoryTransform` to its optic kind. When a protolens chain is classified as `Iso`, the engine skips complement computation entirely — a significant optimization for the common case where schema changes are renames and lossless coercions.
...
1. **Inverse cancellation**: `rename(A,B)` followed by `rename(B,A)` → removed.
2. **Rename fusion**: `rename(A,B)` followed by `rename(B,C)` → `rename(A,C)`.
3. **Add-drop cancellation**: `add(X)` followed by `drop(X)` → removed.
