...
This chapter covers the asymmetric lens primitives (`get` and `put`), the complement type, [Cambria](https://www.inkandswitch.com/cambria/)-style combinators for building lenses declaratively, and the lens laws that guarantee correctness.

The crate lives at `crates/panproto-lens` and is published as `panproto-lens` on crates.io. It depends on `panproto-schema` for schema types, `panproto-inst` for tree instances and compiled migrations, and `panproto-mig` for migration composition. It is a Level 2 crate in panproto's dependency hierarchy.

## Why Lenses?

Migrations (@sec-migration-engine) handle the common case where a schema evolution preserves all information---renames, reorderings, and bijective restructurings. But many real-world evolutions are lossy: removing a deprecated field, collapsing two fields into one, or changing a field's type. These transformations cannot be inverted by `panproto-mig` because there is no unique preimage.
...
::: {.callout-note}
The lens laws follow the formulation of asymmetric lenses from @foster2007, adapted to tree instances over GAT-based schema theories. The complement corresponds to the abstract complement in the lens framework of @johnson2013.^[Complement-based lenses have two useful structural properties: there is an identity lens (that does nothing), and lenses compose associatively (chaining A-to-B and B-to-C produces A-to-C) [@hofmann2011]. panproto exploits this by allowing lens composition through `Compose` combinators and through direct composition of compiled migrations.]
:::
...
In addition to the six original combinators, seven naming combinators operate on the nine naming sites described in @sec-naming-identity:
...
You never need to manually construct vertex maps, edge maps, or resolvers when using combinators. The target schema and migration are derived automatically from the combinator chain. Multi-step chains work correctly because each combinator operates on the output schema of the previous step---a rename followed by a hoist that references the renamed name will find the name as expected.

::: {.callout-tip}
Combinators are inspired by the [Cambria](https://www.inkandswitch.com/cambria/) lens language [@clarke2020]. panproto's combinators will feel familiar if you know Cambria's JSON lens combinators---but they operate on the graph-level schema representation rather than on JSON paths, making them protocol-agnostic. The same combinator chain works for JSON Schema, Protobuf, SQL, or any other supported protocol.
:::

## Design Rationale

The asymmetric lens design follows the theoretical framework of @hofmann2011. The choice of asymmetric (rather than symmetric) lenses is deliberate. In protocol evolution, one direction is typically authoritative---the new schema version defines the "view," and the old schema's extra structure is captured as complement. Symmetric lenses would require both sides to carry complements, adding complexity without clear benefit for the schema migration use case.
...
For a hands-on introduction to building and applying lenses, see [the tutorial](https://panproto.dev/tutorial/chapters/08-lenses.html).
...
- **`Protolens`**: A schema-parameterized lens---a compatible family of maps between theory-level transformations whose components are lenses.^[A protolens is a reusable lens template: given any schema that satisfies a precondition, it produces a concrete lens. Categorically, this is a natural transformation between theory endofunctors. See the tutorial's [Appendix A](https://panproto.dev/tutorial/appendices/A-formal-foundations.html).]
...
The `Protolens::instantiate` method is the `complete` operation: given a schema and protocol, it applies the source and target transformations to compute source and target schemas, derives a `CompiledMigration` between them, and returns a concrete `Lens`.^[Instantiation takes a schema-parameterized template and produces a concrete lens for a specific schema. In dependent type theory, this is called $\Pi$-elimination.]

For full details, see @sec-protolens-engine.

### Relationship to `combinators.rs`

The `Combinator` enum in `combinators.rs` operates at the schema level---each combinator directly mutates a schema and produces a migration step. Protolenses operate at the theory level---each protolens describes a transformation on theories that induces schema transformations when instantiated.

The two layers coexist: `combinators.rs` remains the preferred API for hand-written lens chains (where the user specifies exact field names), while `protolens.rs` is used by the automated pipeline (@sec-auto-lens-pipeline) where lenses are derived from schema diffs.

::: {.callout-note}
The `combinators.rs` module is not deprecated. It provides a simpler, more direct API for cases where the user knows exactly which fields to rename, add, or remove. Protolenses are the right tool when the transformation must be derived automatically or must be schema-generic.
:::
...
The crate root re-exports `Lens`, `Complement`, `get`, `put`, `Combinator`, `from_combinators`, `Protolens`, `ProtolensChain`, `ComplementConstructor`, and the composition functions for convenient access.
