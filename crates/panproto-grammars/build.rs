...
fn main() {
    ...
    let generated = generate_rust_bindings(&compiled, &grammars_dir, &out_dir);
...
/// Resolve a tags.scm with nvim-treesitter-style `;inherits: lang1,lang2`
/// directives. The directive must appear on the first line; inherited
/// languages' resolved tags.scm content is prepended in reverse order. Non-
/// existent inherited languages are ignored with a warning (a grammar may
/// inherit from a language that isn't enabled in the current feature set).
fn resolve_tags_query(name: &str, grammars_dir: &Path) -> String {
    let path = grammars_dir.join(name).join("queries").join("tags.scm");
    let mut seen = std::collections::BTreeSet::new();
    seen.insert(name.to_owned());
    resolve_tags_inner(&path, grammars_dir, &mut seen)
}

/// Implicit inheritance table for grammars whose upstream `tags.scm` omits a
/// `;inherits:` directive but relies on a parent language's captures at
/// runtime (the nvim-treesitter and Helix editors add the directive in their
/// overlay queries). Keys are panproto grammar names; values are parent
/// grammar names in child-first order.
fn implicit_inherits(name: &str) -> &'static [&'static str] {
    match name {
...
fn resolve_tags_inner(
    ...
    // Determine parent grammars via either the explicit `;inherits:`
    // directive on the first line, or the implicit inheritance table below.
    let mut first_line = content.lines().next().unwrap_or("").trim_start();
    first_line = first_line.trim_start_matches(';').trim_start();
    let inherits_found = first_line.to_lowercase().starts_with("inherits");
    ...
    let parents: Vec<String> = if inherits_found {
        first_line
            .split_once(':')
            .map(|x| x.1)
            .map(|list| {
    ...
    for parent in parents.iter().rev() {
        ...
        let parent_path = grammars_dir.join(parent).join("queries").join("tags.scm");
        if parent_path.exists() {
            println!("cargo:rerun-if-changed={}", parent_path.display());
            let resolved = resolve_tags_inner(&parent_path, grammars_dir, seen);
    ...
    let body = if inherits_found {
        // Strip the directive line from the child before appending.
...
fn generate_rust_bindings(
    ...
    for (name, _spec) in &available {
        ...
        // consumed by GitHub code navigation and the `tree-sitter tags` CLI.
        //
        // Honours the nvim-treesitter `;inherits: lang1,lang2` directive on
        // the first line: inherited languages' tags.scm content is prepended
        // in reverse order so later definitions override earlier ones (the
        // tree-sitter query runtime unions patterns; ordering matters only
        // for `#set!` predicates).
        let tags_path = grammars_dir.join(name).join("queries").join("tags.scm");
        if tags_path.exists() {
            println!("cargo:rerun-if-changed={}", tags_path.display());
            let resolved = resolve_tags_query(name, grammars_dir);
            let out_tags = out_dir.join(format!("tags_{}.scm", name.replace('-', "_")));
            fs::write(&out_tags, &resolved).expect("failed to write resolved tags.scm");
            let tags_str = out_tags.display().to_string().replace('\\', "/");
            code.push_str(&format!(
                "const {const_name}_TAGS_QUERY: Option<&str> = Some(include_str!(\"{tags_str}\"));\n",
            ));
        } else {
            code.push_str(&format!(
                "const {const_name}_TAGS_QUERY: Option<&str> = None;\n",
            ));
        }
    }
