...
impl Term {
    ...
    #[must_use]
    pub fn app(op: impl Into<String>, args: Vec<Self>) -> Self {
        Self::App {
            op: op.into(),
            args,
        }
    ...
    #[must_use]
    pub fn substitute(&self, subst: &rustc_hash::FxHashMap<String, Self>) -> Self {
        match self {
    ...
    #[must_use]
    pub fn free_vars(&self) -> rustc_hash::FxHashSet<String> {
        let mut vars = rustc_hash::FxHashSet::default();
        self.collect_vars(&mut vars);
        vars
    }

    fn collect_vars(&self, vars: &mut rustc_hash::FxHashSet<String>) {
        match self {
            Self::Var(name) => {
    ...
    #[must_use]
    pub fn rename_ops(&self, op_map: &std::collections::HashMap<String, String>) -> Self {
        match self {
            Self::Var(_) => self.clone(),
            Self::App { op, args } => Self::App {
                op: op_map.get(op).cloned().unwrap_or_else(|| op.clone()),
...
mod tests {
    ...
    fn term_substitution() {
        ...
        subst.insert("x".into(), Term::var("y"));
