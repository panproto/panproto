...
mod tests {
    ...
    fn import_multi_file_project() {
        let (_dir, git_repo) = create_test_git_repo(&[
            ("src/main.ts", b"function main(): void { console.log('hello'); }"),
            ("src/utils.ts", b"export function add(a: number, b: number): number { return a + b; }"),
    ...
    fn import_multiple_commits() {
        ...
        let tree_oid = index.write_tree().unwrap();
        let tree = repo.find_tree(tree_oid).unwrap();
        let commit1_oid = repo.commit(Some("HEAD"), &sig, &sig, "First", &tree, &[]).unwrap();
        ...
        repo.commit(Some("HEAD"), &sig, &sig, "Second", &tree, &[&commit1]).unwrap();
