...
// ---------------------------------------------------------------------------
// Benchmarks against real AT Protocol records
// ---------------------------------------------------------------------------
...
fn real_post_schema() -> Schema {
    ...
    panproto_protocols::web_document::atproto::parse_lexicon(&value).expect("lex schema")
}
...
fn wtype_restrict_identity_real_post(bencher: Bencher) {
    use panproto_schema::Edge as _Edge;
    let schema = real_post_schema();
    let registry = panproto_io::default_registry();
    ...
    let surviving_edges: HashSet<_Edge> = schema.edges.keys().cloned().collect();
    let compiled = CompiledMigration {
        surviving_verts,
        surviving_edges,
        vertex_remap: HashMap::new(),
